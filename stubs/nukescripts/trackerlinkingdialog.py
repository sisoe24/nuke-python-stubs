...
class LinkToTrackPanel(nukescripts.PythonPanel):
    def __init__(self, groupContext):
        nukescripts.PythonPanel.__init__(
            self, 'Link to Tracker', 'uk.co.thefoundry.LinkToTrackPanel')
        ...
        self._link_to = nuke.Enumeration_Knob(
            'link_to', 'link to', ['position', 'translate', 'translate as offset'])
    ...
    def _updateExpression(self):
        ...
        if self._link_to.value() == 'translate':
            ...
            exprX = ' + '.join([self._tracker.value() + '.' + point.name() +
                               '.track_x' for point in self._track_points if point.value()])
            exprY = ' + '.join([self._tracker.value() + '.' + point.name() +
                               '.track_y' for point in self._track_points if point.value()])
