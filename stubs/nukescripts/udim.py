...
class UDIMOptionsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, parsing_func=parseUdimFile, udim_label='UDIM'):
        ...
        # Setup tooltip
        self.PostageStampCheckBox.setToolTip(
            udimStr('Enable the node postage stamp generation for all {0} files.', self.UdimLabel))
        ...
        hlayout = QtWidgets.QHBoxLayout()
        hlayout.insertSpacerItem(0, QtWidgets.QSpacerItem(
            1, 1, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum))
        hlayout.addWidget(self.OkButton)
    ...
    def updateTableWidget(self):
        ...
        if self.UdimConflict:
            ...
            self.ConflictLabel.setText(udimStr(
                'Conflict : multiple files with the same {0} number. Disable unnecessary files.', self.UdimLabel))
        ...
        for u in self.UdimMap:
            ...
            if u.uv == None:
                ...
                udim_id = QtWidgets.QTableWidgetItem(
                    '(%(u)02d,%(v)02d)' % {'u': u_coord, 'v': v_coord})
    ...
    def importUdimFiles(self):
        ...
        for f in files:
            ...
            for s in sequences:
                ...
                if checkUdimValue(udim) == False:
                    self.reject()
                    self.ErrorMsg = udimStr(
                        'Error. Wrong type returned by {0} parsing function.', self.UdimLabel)
        ...
        if len(warning_msg) > 0:
            errorMsg = udimStr(
                'The following files do not contain a valid {0} number: \n\n' + warning_msg,  self.UdimLabel)
