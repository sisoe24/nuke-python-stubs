...
class ExecuteDialog(nukescripts.PythonPanel):
    ...
    def _addViewKnob(self):
        ...
        if (oc.viewcount() > 2):
            ...
            self._viewSelection.fromScript(self._state.get(
                self._viewSelection, self._getDefaultViews()))
    ...
    def _addKnobs(self):
        ...
        self._rangeEnum = nuke.CascadingEnumeration_Knob(
            'frame_range', 'Frame range', specialRanges)
        ...
        if (source == _FRAME_RANGE_INPUT):
            try:
                ...
                self._frameRange.setValue(
                    str(nuke.activeViewer().node().upstreamFrameRange(activeInput)))
            except:
...
class RenderDialog(ExecuteDialog):
    ...
    def _showDeprecatedWarningMessage(self):
        if not RenderDialog.deprecatedWarningShown:
            ...
            nuke.tprint(
                'Render in Background option functionality has been deprecated and replaced with the Frame Server.')
        ...
        if nuke.Root().name() == 'Root':
            ...
            if nuke.Root().modified() == True or forceSaveNew:
                ...
                try:
                    ...
                    nuke.tprint(
                        'Exception when saving script to: %s. Saving to NUKE_TEMP_DIR' % filePath)
    ...
    def renderToFrameServer(self, frame_ranges, views):
        ...
        def sortRenderOrder(w): return w.knobs()['render_order'].getValue()
        if nodesToRender:
...
class FlipbookDialog(RenderDialog):
    ...
    def _requireIntermediateNode(self, nodeToTest):
        if nodeToTest.Class() == 'Read' or nodeToTest.Class() == 'Write':
            ...
            flipbookSupportsFileType = (supportedFileTypes == [
                                        '*']) or (fileExtension in supportedFileTypes)
    ...
    def _getBurninWriteColorspace(self):
        ...
        if rootNode:
            ...
            if colorManagementKnob:
                # Note: Must use value() rather than getValue().
                usingNukeColorspaces = (colorManagementKnob.value() == 'Nuke')
        ...
        if usingNukeColorspaces:
            ...
            if matchResult is not None:
                ...
                if rootNode:
                    writeColorspace = rootNode.getOCIOColorspaceFromViewTransform(
    ...
    def _createIntermediateNode(self):
        ...
        if self._node.Class() == 'Viewer':
            fixup.setInput(0, self._node.input(
        ...
        if (flipbookToRun):
            if not os.access(flipbookToRun.path(), os.X_OK):
                raise RuntimeError('%s cannot be executed (%s).' %
                                   (flipbookToRun.name(), flipbookToRun.path(),))
            ...
            raise RuntimeError('No flipbook called ' + self._flipbookEnum.value() +
...
def showRenderDialog(nodesToRender, exceptOnError=True, allowFrameServer=True):
    ...
    groupContext = nuke.root()
    d = RenderDialog(_gRenderDialogState, groupContext,
                     nodesToRender, exceptOnError, allowFrameServer)
    _showDialog(d)
