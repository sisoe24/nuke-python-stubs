...
class PrecompOptionsDialog(panels.PythonPanel):
    def __init__(self):
        ...
        self.origNodes = nuke.Enumeration_Knob('orig', 'Original nodes ', [
                                               'add backdrop', 'delete', 'no change'])
...
def precomp_selected():
    ...
    if len(expressionDeps) > 0:
        addLinkedExpressionNodes = nuke.ask(
            'Warning: The selected nodes have expressions to nodes outside the precomp. Do you want to copy these nodes to the precomp?')
    ...
    with group:
        try:
            ...
            nuke.message(
                "Could not write precomp script, permission denied, please specify a different \'script path\' and try again.")
            writeOk = False
    ...
    if len(inputs) > 0:
        nuke.message(
            'Warning: The precomp script requires inputs and may not render the same independent of its parent script.')
