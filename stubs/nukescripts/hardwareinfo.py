...
class HardwareInfo:
    ...
    def parseMemInfo(self):
        ...
        if success:
            self.MapDictionaries(itemDict, self._standardDict, [
                                 ['MemTotal', gRAM]], '/proc/meminfo')
    ...
    def initWin(self):
        ...
        self._standardDict[gCPUType] = self.getRegistryValue(
            'HKEY_LOCAL_MACHINE', 'HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0', 'ProcessorNameString')
        self._standardDict[gCPUSpeed] = self.getRegistryValue(
            'HKEY_LOCAL_MACHINE', 'HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0', '~MHz')
        self._standardDict[gNumCores] = self.getRegistryNumSubKeys(
        ...
        for key in sorted(pDict):
            ...
            if type(elem) == dict:  # nest
                ...
                try:
                    ...
                    handle.write(
                        '<%(key)s>[--- unicode decoding problem ---]</%(key)s>' % {'key': key})
            handle.write('\n')
