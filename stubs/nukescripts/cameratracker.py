...
def populateExportMenu(cameraTracker):
    ...
    entries = [
        ...
        ('Camera',               'nukescripts.cameratracker.createCamera(nuke.thisNode())',
         'Create a Camera linked to the calculated projection.'),
        ('Camera rig',           'nukescripts.cameratracker.createCameraRig(nuke.thisNode())',
         'Create a multi-view rig with a Camera per view.'),
        ('Camera set',           'nukescripts.cameratracker.createCameraSet(nuke.thisNode())',
         'Create a set of Cameras, one Camera per solved frame.'),
        ('Scene',                'nukescripts.cameratracker.createScene(nuke.thisNode())',
         'Create a Scene with a Camera and PointCloud for the camera solve.'),
        ('Scene+',                'nukescripts.cameratracker.createEverything(nuke.thisNode())',
         'Create a Scene with a Camera, PointCloud, ScanlineRender, and LensDistortion (Undistort) node.'),
        ('Point cloud',          'nukescripts.cameratracker.createPointCloud(nuke.thisNode())',
         'Create a PointCloud for the camera solve.'),
        ...
        ('Cards',                'nukescripts.cameratracker.createCards(nuke.thisNode())',
    ...
    tooltipLines = [
        'Create new camera and point cloud nodes based on the results of the solve. The available options are:', '']
...
def populateFilmBackPresets(cameraTracker):
    ...
    k.setValues(['%s\t%s' % (idx, label)
                for idx, label in enumerate(nukescripts.camerapresets.getLabels())])
    ...
    for i in range(numviews):
        ...
        if link:
            ...
            if solver.knob('camTranslate').isAnimated():
                camera.knob('translate').copyAnimation(
                    0, solver.knob('camTranslate').animation(0, i+1))
                camera.knob('translate').copyAnimation(
                    1, solver.knob('camTranslate').animation(1, i+1))
                camera.knob('translate').copyAnimation(
                    2, solver.knob('camTranslate').animation(2, i+1))
            else:
                camera.knob('translate').setValue(solver.knob(
                    'camTranslate').getValue(0, i+1), 0, 0, i+1)
                camera.knob('translate').setValue(solver.knob(
                    'camTranslate').getValue(1, i+1), 1, 0, i+1)
                camera.knob('translate').setValue(solver.knob(
...
def createCameraSet(solver):
    ...
    if numCameras >= 100:
        ok = nuke.ask(
            'This will create %d cards, which may take some time.\nAre you sure?' % numCameras)
        if not ok:
            return
    ...
    for i in range(numCameras):
        ...
        if link:
            camera.knob('focal').setExpression(
                exprStr + ".knob('focalLength').getValueAt(" + str(frame) + ')}]')
            camera.knob('haperture').setExpression(
                exprStr + ".knob('aperture').getValueAt(" + str(frame) + ',0)}]')
            camera.knob('vaperture').setExpression(
                exprStr + ".knob('aperture').getValueAt(" + str(frame) + ',1)}]')
            camera.knob('translate').setExpression(
                exprStr + ".knob('camTranslate').getValueAt(" + str(frame) + ',0)}]', 0)
            camera.knob('translate').setExpression(
                exprStr + ".knob('camTranslate').getValueAt(" + str(frame) + ',1)}]', 1)
            camera.knob('translate').setExpression(
                exprStr + ".knob('camTranslate').getValueAt(" + str(frame) + ',2)}]', 2)
            camera.knob('rotate').setExpression(
                exprStr + ".knob('camRotate').getValueAt(" + str(frame) + ',0)}]', 0)
            camera.knob('rotate').setExpression(
                exprStr + ".knob('camRotate').getValueAt(" + str(frame) + ',1)}]', 1)
            camera.knob('rotate').setExpression(
...
def _createLensDistortionNode(cameraTracker, invertDistortion):
    ...
    _copyKnob(cameraTracker, 'lensType',
              lensDistort, 'lensType',                 link)
    _copyKnob(cameraTracker, 'distortion1',
              lensDistort, 'distortionDenominator0',   link)
    _copyKnob(cameraTracker, 'distortion2',
              lensDistort, 'distortionDenominator1',   link)
    _copyKnob(cameraTracker, 'distortionCenter',
              lensDistort, 'centre',                   link)
    _copyKnob(cameraTracker, 'anamorphicSqueeze',
              lensDistort, 'anamorphicSqueeze',        link)
    _copyKnobIdx(cameraTracker, 'asymmetricDistortion', 0,
                 lensDistort, 'distortionDenominatorX00', 0, link)
    _copyKnobIdx(cameraTracker, 'asymmetricDistortion', 1,
                 lensDistort, 'distortionDenominatorY00', 0, link)
...
def createCards(solver):
    ...
    if numCameras > 0:
        ...
        zDistKnob.setTooltip(
            'Cards are placed this far from origin. Use this to make a pan & tile dome of this radius.')
...
def _copyLinkableXYAnimCurve(linkableSrc, linkableDst):
    ...
    for i in range(0, linkKnobSrc.getNumKeys(int(linkableSrcIndices[0]))):
        ...
        linkKnobDst.setValueAt(linkKnobSrc.getValueAt(
            t, int(linkableSrcIndices[0])), t, int(linkableDstIndices[0]))
        linkKnobDst.setValueAt(linkKnobSrc.getValueAt(
            t, int(linkableSrcIndices[1])), t, int(linkableDstIndices[1]))
