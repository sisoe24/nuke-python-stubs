...
class FramecyclerFlipbook(flipbooking.FlipbookApplication):
    ...
    def run_app(self, app, in_args):
        ...
        for a in in_args:
            args.append(a)
        try:
            p = subprocess.Popen(args=args, executable=app,
...
flipbooking.registerLUTPath('FrameCycler', 'linear-sRGB',
                            os.path.join(fcBasePath, 'LUTs', 'NukeLinearTosRGB.ilut'))
flipbooking.registerLUTPath('FrameCycler', 'linear-rec709',
                            os.path.join(fcBasePath, 'LUTs', 'NukeLinearToRec709.ilut'))
flipbooking.registerLUTPath('FrameCycler', 'Cineon-sRGB',
                            os.path.join(fcBasePath, 'LUTs', 'NukeCineonTosRGB.cube'))
