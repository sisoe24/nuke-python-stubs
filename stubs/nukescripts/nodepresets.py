...
class CreateNodePresetsPanel(nukescripts.PythonPanel):
    def __init__(self, node):
        ...
        self.menuItemChoice.setTooltip(
            "The menu location that the Preset will appear in. Specify 'root' to place the Preset in the main Presets menu.")
...
def populatePresetsMenu(nodeName, className):
    ...
    def buildSavePanelFunc(node=node): return nukescripts.buildPresetSavePanel('', node)
    m.addCommand('Save as Preset', buildSavePanelFunc)

    if allpresets != []:
        m.addCommand('-', '', '')
        for k in userpresets:
            def f(node=node, k=k): return nuke.applyUserPreset('', k, node)
            m.addCommand('[User] ' + k, f)
        for k in presets:
            def f(node=node, k=k): return nuke.applyPreset('', k, node)
            m.addCommand(k, f)
    if allpresets != []:
        m.addCommand('-', '', '')
