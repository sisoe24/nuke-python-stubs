...
from hiero.core.log import info, debug
...
class track:
    ...
    def getTrackId(self):
        if self.__trackId == -1:
            self.__trackId = FnCyclone.addTrack(parentTimeline=self.__timelineId, data={
                                                FnCyclone.kTrackType: self.__trackType, FnCyclone.kTrackName: self.__name, FnCyclone.kTrackEdlName: self.__filename})
...
class EdlImporter:
    ...
    def extractFilenamesAndClipName(self, element):
        ...
        if element.hasElement('PATH') and element.hasElement('FILENAME'):
            ...
            filename = os.path.join(element.getElement(
                'PATH')[0], element.getElement('FILENAME')[0])
    ...
    def createClipAndMedia(self, element, mediaDict, trackDict):
        ...
        for filename in filenames:
            ...
            if not index < len(trackDict):
                debug('FnEdlImporter.createClipAndMedia out of range index: ' +
                      str(index) + ' ' + str(len(trackDict)))
                break
            ...
            if mediaidentifier in mediaDict:
                ...
                # New Media
                mediaId = mediaDict[mediaidentifier] = FnCyclone.addMedia(
                    data=self.getMediaData(filename, element, srcDuration, startTC))
    ...
    def createTransition(self, prevelement, element, clipDict):
        ...
        if not prevelement or prevelement.isBlank():
            ...
            if element.isBlank():
                info('Error : Invalid Transition from %s to %s',
                     prevelement.editId(), element.editId())
        # TO Clip is blank
        ...
        data = {FnCyclone.kTransitionInDuration: inDuration,
                FnCyclone.kTransitionOutDuration: outDuration, FnCyclone.kTransitionExclusive: 1}
        ...
        for element in edl:
            ...
            if isinstance(element, EditDecision):
                ...
                # If an element is blank but has a FROMCLIPNAME, it's a fade out. Move it to the previous element.
                    ...
                    if not prevelement.hasElement(FROMCLIPNAME):
                        prevelement.addElement(NamedElement(
                            FROMCLIPNAME, element.getElement(FROMCLIPNAME)))
                        element.removeElement(FROMCLIPNAME)
                    else:
