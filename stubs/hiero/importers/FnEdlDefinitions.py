...
RETIME_DIRECTIVE = Literal('M2') + SOURCE_IDENTIFICATION_FIELD + \
    REAL_NUMBER + Optional(SOURCE_ENTRY_FIELD)
...
SYSTEM_DIRECTIVE = ((Literal('TITLE') | Literal('FCM')) + Optional(Literal(':')
...
SOURCEFILE_ELEMENT = (Combine(Literal('SOURCE') + Literal('FILE') +
                      Optional(Literal('PATH')).hide() + Literal(':').hide()) + Word(NoneOf('\n\r')))
...
#
EDIT_SUB_ELEMENT = OneOrMore((DLEDL_ELEMENT) | (USER_ELEMENT) | (COMMENT_ELEMENT) | (
    RETIME_DIRECTIVE) | (AUD_ELEMENT) | (INFORMATIONAL_DIRECTIVE))

INDEXED_EDIT_ELEMENT = ((INDEXED_SOURCE_ELEMENT) +
                        ZeroOrMore(EDIT_SUB_ELEMENT)) | (INDEXED_SOURCE_ELEMENT)
