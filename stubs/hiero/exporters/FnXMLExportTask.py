...
import hiero.core
from hiero.core import Clip, Keys, Sequence, TimeBase, Timecode, util
...
class XMLExportTask(hiero.core.TaskBase):
    ...
    def addTimecodeElem(self, parentElem, timeBase, timecodeStart, reel=None):
        ...
        etree.SubElement(timecodeElem, 'string').text = hiero.core.Timecode.timeToString(
            timecodeStart, timeBase, hiero.core.Timecode.kDisplayTimecode)
    ...
    def samplesToFrames(self, source, samples):
        ...
        sampleRate = hiero.core.TimeBase.fromString(
            source.metadata().value(Keys.kSourceSamplerate)).toFloat()
    ...
    def addFileElem(self, parentElem, source, clip):
        ...
        etree.SubElement(fileElem, 'duration').text = str(
            self.sourceTime(source, source.duration()))
