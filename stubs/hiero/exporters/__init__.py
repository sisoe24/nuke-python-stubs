...
def AddDefaultPresets(overwrite):
    ...
    shotTemplateRenderPreset = FnExternalRender.NukeRenderPreset(
    ...
    shottranscodeTemplate = (('{shot}/{shot}.####.{ext}', FnTranscodeExporter.TranscodePreset(
        '', {'file_type': 'dpx', 'dpx': {'datatype': '10 bit'}})), )

    dpxTranscodePreset = FnTranscodeExporter.TranscodePreset(
        '', {'file_type': 'dpx', 'dpx': {'datatype': '10 bit'}})
    sequencetemplate = (('{sequence}_{version}/{sequence}_{version}.####.{ext}',
                        registry.copyPreset(dpxTranscodePreset)), )
    sequenceMultiViewTemplate = (
    ...
    processorPresets = {'Basic Nuke Shot':
                        ...
                        (FnShotProcessor.ShotProcessorPreset, {
                         'exportTemplate': shotannotationstemplate, 'cutLength': True}),
                        'Multi-View Nuke Shot (%v)':
                        (FnShotProcessor.ShotProcessorPreset, {
                         'exportTemplate': multiViewShotTemplate, 'cutLength': True}),
                        'Transcode Shots DPX':
                        (FnShotProcessor.ShotProcessorPreset, {
                         'exportTemplate': shottranscodeTemplate, 'cutLength': True}),
                        'Log10 Cineon DPX':
                        (FnTimelineProcessor.TimelineProcessorPreset,
                         {'exportTemplate': sequencetemplate}),
                        'Log10 Cineon DPX Multi-View':
                        (FnTimelineProcessor.TimelineProcessorPreset, {
                            'exportTemplate': sequenceMultiViewTemplate}),
                        'Transcode Clips DPX':
                        (FnBinProcessor.BinProcessorPreset,
                         {'exportTemplate': clipstemplate}),
                        }
    ...
    if not nukeEnv['ple']:
        processorPresets['CMX 3600 EDL'] = (FnTimelineProcessor.TimelineProcessorPreset, {
