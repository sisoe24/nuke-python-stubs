...
class NukeAnnotationsExporter(NukeShotExporter):
    ...
    def getSequenceAnnotationsKeyFrames(self):
        ...
        for track in connectedTracks:
            ...
            trackAnnotations = [item for item in itertools.chain(
                *track.subTrackItems()) if isinstance(item, hiero.core.Annotation)]
    ...
    def getTrackItemAnnotationsKeyFrames(self):
        ...
        keyFrames.update(self.getAnnotationKeyFramesForTrackItem(
            trackItem, clipAnnotationsOffset, toSequenceTime))
    ...
    def _beforeNukeScriptWrite(self, script):
        ...
        if annotationKeyFrames:
            annotationKeysStr = ''.join([' x%s %s' % (frame, index + 1)
                                        for index, frame in enumerate(annotationKeyFrames)])
