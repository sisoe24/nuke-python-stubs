...
from . import FnScriptLayout, FnShotExporter, FnEffectHelpers, FnExternalRender
from .FnExportUtil import (TrackItemExportScriptWriter, createViewerNode,
                           trackItemTimeCodeNodeStartFrame)
from .FnReformatHelpers import (reformatNodeFromPreset,
                                reformatSettingsFromPreset)
...
class NukeShotExporter(FnShotExporter.ShotTask):
    ...
    def __init__(self, initDict):
        ...
        if isinstance(self._item, hiero.core.TrackItem):
            ...
            if len(self._collatedItems) > 1:
                ...
                self._effects, self._annotations = FnEffectHelpers.findEffectsAnnotationsForTrackItems(
                    self._collatedItems)
        ...
        if _nuke.env['nc'] or _nuke.env['indie']:
            ...
            for index in range(len(annotationsPreCompPaths)):
                if annotationsPreCompPaths[index].endswith('.nk'):
                    annotationsPreCompPaths[index] = self._changeExtension(
                        annotationsPreCompPaths[index])
        ...
        if self._collate:
            ...
            for trackitem in self._collatedItems:
                ...
                if self._cutHandles is not None:
                    ...
                    handleIn, handleOut = min(self._cutHandles, handleInLength), min(
                        self._cutHandles, handleOutLength)
                    ...
                    if trackItemCopy.timelineIn() <= sequenceIn and handleIn:
                        ...
                        hiero.core.log.debug('Expanding %s in by %i frames' %
                    ...
                    if trackItemCopy.timelineOut() >= sequenceOut and handleOut:
                        ...
                        hiero.core.log.debug('Expanding %s out by %i frames' %
            ...
            if copiedEffects and (usingPlateFormat or usingCustomFormat):
                ...
                effectsFormatChange = FormatChange(
                    masterItemFormat, self._sequence.format(), masterItemFormat, self._item.reformatState())
                ...
                if usingCustomFormat:
                    reformatType, format, scale, resize, center, filter = reformatSettingsFromPreset(
                        self._preset)
                    reformatState = {'type': reformatType,
                                     'resizeType': resize, 'resizeCenter': center}
    ...
    def updateItem(self, originalItem, localtime):
        ...
        isFreezeFrame = isinstance(
            self._item, hiero.core.TrackItem) and self._item.playbackSpeed() == 0.0
        ...
        if self._preset.properties()['additionalNodesEnabled']:
            if isinstance(item, hiero.core.Clip):
                ...
                nodes.extend(FnExternalRender.createAdditionalNodes(
                    FnExternalRender.kPerShot, data, item))
                ...
                nodes.extend(FnExternalRender.createAdditionalNodes(
                    FnExternalRender.kPerTrack, data, item))
            elif isinstance(item, hiero.core.Sequence):
                # Use AdditionalNodes data to populate based on sequence tags
                nodes.extend(FnExternalRender.createAdditionalNodes(
                    FnExternalRender.kPerSequence, data, item))
        return nodes
    ...
    def _createAnnotationsPreComps(self):
        ...
        for path in self._preset.properties()['annotationsPreCompPaths']:
            ...
            precomp.addRawKnob(
                'addUserKnob {41 annotation_key l annotation T AnnotationsKeys.annotation_key}')
            precomp.addRawKnob(
                'addUserKnob {41 annotation_count l of -STARTLINE T AnnotationsKeys.annotation_count}')
            precomp.addRawKnob(
    ...
    def writeSequence(self, script):
        ...
        script.pushLayoutContext('sequence', self._sequence.name(),
    ...
    def _taskStep(self):
        ...
        if self._preset.properties()['additionalNodesEnabled']:
            script.addNode(FnExternalRender.createAdditionalNodes(
                FnExternalRender.kUnconnected, self._preset.properties()['additionalNodesData'], self._item))
        ...
        script.addNode(nuke.AddTimeCodeNode(timecodeStart=timecodeStart,
        ...
        if self._preset.properties().get('postProcessScript', True):
            ...
            if error:
                hiero.core.log.error(
                    'Script Post Processor: An error has occurred while preparing script:\n%s', scriptFilename)
    ...
    def outputRangeForTrackItem(self, ignoreHandles=False, ignoreRetimes=True, clampToSource=True):
        ...
        # Offset by custom start time
            ...
            if ignoreHandles and self._cutHandles:
                inputRangeWithHandles = self.inputRange(
                    ignoreHandles=False, ignoreRetimes=ignoreRetimes, clampToSource=clampToSource)
