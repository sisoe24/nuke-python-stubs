...
from . import (FnAudioHelper, FnAudioConstants, FnExternalRenderUI,
               FnTranscodeExporter, FnAdditionalNodesDialog)


class TranscodeExporterUI(FnExternalRenderUI.NukeRenderTaskUI):
    def __init__(self, preset):
        """Initialize"""
        FnExternalRenderUI.NukeRenderTaskUI.__init__(
            self, preset, FnTranscodeExporter.TranscodeExporter, 'Transcode Images')
        self._tags = []
    ...
    def populateUI(self, widget, exportTemplate):
        ...
        uiProperty = UIPropertyFactory.create(type(
            value), key=key, value=value, dictionary=self._preset._properties, label='Retime Method', tooltip=retimeToolTip)
        ...
        uiProperty = UIPropertyFactory.create(type(value), key=key, value=value, dictionary=self._preset.properties(
        ), label=label+':', tooltip=includeEffectsToolTip)
        ...
        uiProperty = UIPropertyFactory.create(type(value), key=key, value=value, dictionary=self._preset.properties(
        ), label=label+':', tooltip=includeAnnotationsToolTip)
        formLayout.addRow(label+':', uiProperty)
        self._uiProperties.append(uiProperty)
        ...
        codecToolTip = (
            'Audio Codec (If the file type is set to MOV, it will only support the liner PCM codec).')

        key, value = 'includeAudio', False
        includeAudioWidget = UIPropertyFactory.create(
            type(value), key=key, value=value, dictionary=self._preset._properties)
        ...
        FnAudioHelper.createCodecProperty(
            self, self._audioLayout, self.updateAudioSettings, codecToolTip)

        key, value = 'deleteAudio', True
        deleteAudioWidget = UIPropertyFactory.create(
            type(value), key=key, value=value, dictionary=self._preset._properties)
        ...
        keepNukeScriptCheckbox.setToolTip(
            "A Nuke script is created for each transcode. If you'd like to keep the temporary .nk file from being destroyed, enable this option. The script will get generated into the same directory as the transcode output")
        ...
        self._readAllLinesCheckbox.setToolTip(
            'Select this to set up the export script to read all lines from the inputs in one go, where possible (currently, this only applies to dpx inputs). This option can be faster for I/O-heavy export scripts.')
        formLayout.addRow('Read All Lines:', self._readAllLinesCheckbox)

        self._useSingleSocketCheckbox = QtWidgets.QCheckBox()
        checkBoxState = QtCore.Qt.Checked if self._preset.properties(
        )['useSingleSocket'] else QtCore.Qt.Unchecked
        ...
        self._useSingleSocketCheckbox.setToolTip(
            'Select this to set up the export process to be limited to a single cpu socket, this can increase performance. This option is only available for machines with multiple cpu sockets and when using Single Render Process.')
    ...
    def _enableReadAllLinesForCodec(self):
        ...
        self._readAllLinesCheckbox.setCheckState(checkBoxState)
        assert (self._readAllLinesCheckbox.isChecked() ==
                self._preset.properties()['readAllLinesForExport'])
    ...
    def _additionalNodesEditClicked(self):
        dialog = FnAdditionalNodesDialog.AdditionalNodesDialog(
            self._preset.properties()['additionalNodesData'], self._tags)
        if dialog.exec_():
