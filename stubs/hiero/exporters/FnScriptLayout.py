    ...
    def __init__(self, script, layoutContext):
        ...
        for childContext in layoutContext.getChildren():
            if layoutContext.getChildren() or layoutContext.getNodes():
                ...
                assert layoutClass, 'No layout for context: %s %s' % (
                    childContext.getType(), childContext.getName())
...
class ClipLayout(LayoutBase):
    ...
    def doLayout(self, xpos, ypos, zorder):
        ...
        if self.constantNodes:
            ...
            mainNodesRect = self.layoutNodes(
                self.clipNodes, mainNodesXPos, ypos, LayoutBase.kAlignLeft)

            constNodesXPos = xpos
            constNodesRect = self.layoutNodes(
                self.constantNodes, constNodesXPos, ypos, LayoutBase.kAlignLeft)
