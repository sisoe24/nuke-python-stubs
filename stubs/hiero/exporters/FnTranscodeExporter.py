...
from . import (FnAudioHelper, FnScriptLayout, FnShotExporter, FnEffectHelpers,
               FnAudioConstants, FnExternalRender)
...
class TranscodeExporter(FnExternalRender.NukeRenderTask):
    ...
    def updateItem(self, originalItem, localtime):
        ...
        tag.metadata().setValue('tag.description', 'Transcode ' +
                                self._preset.properties()['file_type'])
    ...
    def writeClipOrTrackItemToScript(self, script):
        ...
        """
        isMovieContainerFormat = self._preset.properties(
        )['file_type'] in ('mov', 'mov64', 'ffmpeg')
...
class TranscodePreset(hiero.core.RenderTaskPreset):
    def __init__(self, name, properties):
        ...
        self.properties()['burninData'] = dict((datadict['knobName'], None)
                                               for datadict in FnExternalRender.NukeRenderTask.burninPropertyData)
