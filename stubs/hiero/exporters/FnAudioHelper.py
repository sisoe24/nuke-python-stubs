...
def setAudioExportSettings(exportTask):
    exportTask._outputChannels = FnAudioConstants.kChannels[exportTask._preset.properties()[
        FnAudioConstants.kNumChannelsKey]]
    exportTask._sampleRate = FnAudioConstants.kSampleRates[exportTask._preset.properties()[
        FnAudioConstants.kSampleRateKey]]
    exportTask._bitDepth = FnAudioConstants.kBitDepths[exportTask._preset.properties()[
        FnAudioConstants.kBitDepthKey]]
    exportTask._bitRate = FnAudioConstants.kBitRates[exportTask._preset.properties()[
        FnAudioConstants.kBitRateKey]]
...
def createCodecSpecificProperties(exportUI, formLayout, enabled):
    ...
    srateKey, srateValue = FnAudioConstants.kSampleRateKey, list(
        FnAudioConstants.kSampleRates.keys())
    srateToolTip = ('Audio Sample Rate.')
    sampleRateProperty = UIPropertyFactory.create(type(
        srateValue), key=srateKey, value=srateValue, dictionary=exportUI._preset._properties, label='Sample Rate')
    ...
    if exportUI._preset.properties()[FnAudioConstants.kCodecKey] == FnAudioConstants.kAC3Codec:
        ...
        if exportUI._preset.properties()[FnAudioConstants.kNumChannelsKey] == FnAudioConstants.kSevenPointOneLayout:
            ...
            )[FnAudioConstants.kNumChannelsKey] = FnAudioConstants.kFivePointOneLayout
        ...
        if exportUI._preset.properties()[FnAudioConstants.kNumChannelsKey] == FnAudioConstants.kSevenPointOneLayout\
            ...
            exportUI._preset.properties(
            )[FnAudioConstants.kNumChannelsKey] = FnAudioConstants.kStereoLayout
        channelsProperty.update()
