...
def _mapDefaultColourTransform(preset, projectsettings):
    ...
    if file_type in mapping_table:
        ...
        if lut is None:
            raise RuntimeError(
                'OCIO : Output colourspace set to Default but project Colour Management settings for < %s > is not set.' % lut_key)
...
class NukeRenderTask(ExternalRenderTask):
    ...
    def addBurninNodes(self, script=None):
        ...
        if self._preset.properties()['burninDataEnabled']:
            ...
            defaults = dict((datadict['knobName'], getDefault(datadict['value']))
                            for datadict in NukeRenderTask.burninPropertyData)
...
class NukeRenderPreset(hiero.core.RenderTaskPreset):
    def __init__(self, name, properties):
        ...
        self.properties()['burninData'] = dict((datadict['knobName'], None)
                                               for datadict in NukeRenderTask.burninPropertyData)
...
NukeRenderTask.burninKeys = (('{sequence}', '[metadata hiero/sequence]'), ('{clip}', '[metadata hiero/clip]'), ('{shot}', '[metadata hiero/shot]'),
                             ('{track}', '[metadata hiero/track]'), ('{project}', '[metadata hiero/project]'), ('{timecode}', '[metadata input/timecode]'))
# Load gizmo from file 'BurnInGroup.nk'. add knobName_value to .nk file where properties should be injected.
NukeRenderTask.burninNodeData = open(os.path.join(
    os.path.dirname(__file__), 'BurnInGroup.nk')).read()
...
NukeRenderTask.burninPropertyData = (
    ...
    {'label': 'Top Left', 'knobName': 'burnIn_topLeft', 'value': CustomList(
        *[token for token, key in NukeRenderTask.burninKeys], default='{clip}')},
    {'label': 'Top Middle', 'knobName': 'burnIn_topMiddle', 'value': CustomList(
        *[token for token, key in NukeRenderTask.burninKeys], default='{sequence}')},
    {'label': 'Top Right', 'knobName': 'burnIn_topRight', 'value': CustomList(
        *[token for token, key in NukeRenderTask.burninKeys], default='{shot}')},
    {'label': 'Bottom Left', 'knobName': 'burnIn_bottomLeft', 'value': CustomList(
        *[token for token, key in NukeRenderTask.burninKeys], default='{track}')},
    {'label': 'Bottom Middle', 'knobName': 'burnIn_bottomMiddle', 'value': CustomList(
        *[token for token, key in NukeRenderTask.burninKeys], default='{project}')},
