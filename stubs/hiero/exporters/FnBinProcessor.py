            ...
            if item.clip() and (item.clip().mediaSource().isMediaPresent() or not self.skipOffline()):
                ...
                for (exportPath, preset) in self._exportTemplate.flatten():
                    ...
                    resolver.addResolver(
                        '{binpath}', 'Path within the bin, up to the selected BinItem. If no BinItem selected, path up to project', binPath)
                    resolver.addResolver(
                        '{fullbinpath}', 'Path within the bin, up to the project', fullBinPath)
