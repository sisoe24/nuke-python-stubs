...
from . import FnNukeShotExporter, FnAdditionalNodesDialog
...
class NukeShotExporterUI(hiero.ui.TaskUIBase):
        ...
        if self._isShotExport():
            if len(presetValue) > 0:
                if self._collateTimeProperty._widget.checkState() == QtCore.Qt.Checked or self._collateNameProperty._widget.checkState() == QtCore.Qt.Checked:
                    ...
                    ), 'Conflicting Options', "Overriding the Read node paths will disable the 'Collate' options. Currently these features are incompatible.")
    ...
    def createNodeSelectionWidget(self, layout, exportTemplate):
        ...
        self._nodeSelectionWidget.getModel(
            NukeProjectNodeSelectionWidget.kReadNode).dataChanged.connect(self.readPresetChanged)
        self._nodeSelectionWidget.getModel(
            NukeProjectNodeSelectionWidget.kWriteNode).dataChanged.connect(self.writePresetChanged)
        self._nodeSelectionWidget.getModel(NukeProjectNodeSelectionWidget.kAnnotationsNode).dataChanged.connect(
            self.annotationsPreCompPresetChanged)
    ...
    def populateUI(self, widget, exportTemplate):
        if exportTemplate:
            ...
            writeModel = self._nodeSelectionWidget.getModel(
                NukeProjectNodeSelectionWidget.kWriteNode)
    ...
    def _addReformatOptions(self, layout):
        ...
        if 'width' in rfProperties and 'height' in rfProperties and 'pixelAspect' in rfProperties and 'name' in rfProperties:
            try:
                format = hiero.core.Format(
                    rfProperties['width'], rfProperties['height'], rfProperties['pixelAspect'], rfProperties['name'])
        ...
        self._reformatPropertyLabels[self._formatChooser] = layout.labelForField(
            self._formatChooser)
        ...
        key, value, label = 'resize', (nuke.ReformatNode.kResizeNone, nuke.ReformatNode.kResizeWidth, nuke.ReformatNode.kResizeHeight,
                                       nuke.ReformatNode.kResizeFit, nuke.ReformatNode.kResizeFill, nuke.ReformatNode.kResizeDistort), 'Resize'
        uiProperty = UIPropertyFactory.create(type(
            value), key=key, value=value, dictionary=rfProperties, label=label, tooltip=ReformatToolTips.resize)
        self._uiProperties.append(uiProperty)
        ...
        uiProperty = UIPropertyFactory.create(type(
            value), key=key, value=value, dictionary=rfProperties, label=label, tooltip=ReformatToolTips.center)
        self._uiProperties.append(uiProperty)
        resizeLayout.addWidget(uiProperty._widget)
        ...
        uiProperty = UIPropertyFactory.create(type(
            value), key=key, value=value, dictionary=rfProperties, label=label, tooltip=ReformatToolTips.filter)
        self._uiProperties.append(uiProperty)
        layout.addRow(uiProperty._label + ':', uiProperty)
        uiProperty.update()
        uiProperty.propertyChanged.connect(self.propertyChanged)
    ...
    def _addCollateOptions(self, layout):
        ...
        uiProperty = UIPropertyFactory.create(type(value), key=key, value=value, dictionary=self._preset.properties(
        ), label=label+':', tooltip=collateTracksToolTip)
        ...
        uiProperty = UIPropertyFactory.create(type(value), key=key, value=value, dictionary=self._preset.properties(
        ), label=label+':', tooltip=collateShotNameToolTip)
        layout.addRow(label+':', uiProperty)
        ...
        key, value, label = 'connectTracks', self._preset.properties()[
            'connectTracks'], 'Connect Tracks'
        uiProperty = UIPropertyFactory.create(type(
            value), key=key, value=value, dictionary=self._preset.properties(), label=label+':', tooltip=tooltip)
        layout.addRow(label+':', uiProperty)
        self._uiProperties.append(uiProperty)
