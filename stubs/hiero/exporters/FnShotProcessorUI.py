...
from .FnShotProcessor import (ShotProcessor, ShotProcessorPreset,
                              buildTagsData, findTrackItemExportTag)
...
class ShotProcessorUI(hiero.ui.ProcessorUIBase, QtCore.QObject):
    ...
    def __init__(self, preset):
        ...
        hiero.ui.ProcessorUIBase.__init__(
            self, preset, itemTypes=hiero.core.TaskPresetBase.kTrackItem)
    ...
    def validate(self, exportItems):
        ...
        if invalidItems:
            ...
            msgBox = QtWidgets.QMessageBox()
            msgBox.setTextFormat(QtCore.Qt.RichText)
            result = msgBox.information(None, 'Export', 'The following items will be ignored by this export:<br/>%s' % str(
                '<br/>'.join(invalidItems)), QtWidgets.QMessageBox.Ok | QtWidgets.QMessageBox.Cancel)
    ...
    def trackSelectionChanged(self, item):
        ...
        for sequence in self._sequences:
            excludedTracks.extend(
                [track for track in sequence if track.name() in excludedTrackNames])
    ...
    def _checkExistingVersions(self, exportItems):
        ...
        for item in trackItems:
            ...
            if tag:
                try:
                    existingExportVersionsFound = self._checkTrackItemExportExistingVersion(
                        item, self._preset.properties()['versionIndex'], tag)
                except:
        ...
        if existingExportVersionsFound:
            ...
            response = QtWidgets.QMessageBox.question(hiero.ui.mainWindow(
            ), title, text, QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
    ...
    def _trackItemsToExport(self, exportItems):
        ...
        if exportItems[0].trackItem():
            trackItems = [item.trackItem() for item in exportItems if isinstance(
                item.trackItem(), hiero.core.TrackItem) and not item.ignore()]
        else:
            sequences = [item.sequence() for item in exportItems]
            for sequence in sequences:
                for track in self._tracksToExport(sequence):
                    trackItems.extend([item for item in track if (
                        item.isMediaPresent() or not self.skipOffline())])
