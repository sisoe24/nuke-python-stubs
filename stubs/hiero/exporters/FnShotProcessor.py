...
from .FnExportKeywords import (KeywordTooltips, kFileBaseKeyword,
                               kFileHeadKeyword, kFilePathKeyword)
...
def buildTagsData(exportItems):
    ...
    tags = [(tag, objecttype) for tag, objecttype in tags if tag.visible()
            and reverse_contains(tag.name(), filters) and uniquetest(tag, objecttype)]
    return tags
...
class ShotProcessor(hiero.core.ProcessorBase):
    ...
    def _getTrackItemExistingExportVersionIndices(self, item, tag):
        ...
        versionIndices = scanner.getVersionIndicesForPath(
            tagScriptPath)  # Get all the existing version indices
    ...
    def startProcessing(self, exportItems, preview=False):
        ...
        for sequence in sequences:
            ...
            if not preview:
                presetId = hiero.core.taskRegistry.addPresetToProjectExportHistory(
                    project, self._preset)
            else:
                presetId = None
                ...
                for trackitem, trackitemCopy in zip(list(track.items()), list(trackCopy.items())):
                    ...
                    if not selectedTrackItems or trackitem in selectedTrackItems:
                        ...
                        if trackitem in ignoredTrackItems:
                            hiero.core.log.debug('%s marked as ignore, skipping. Selection : %s' % (
                                str(trackitemCopy), str(exportTrackItems)))
                            continue
                        ...
                        if included_tag_names:
                            ...
                            if not includedTags or excludedTags:
                                hiero.core.log.debug('%s does not contain include tag %s, skipping.' % (
                            ...
                            hiero.core.log.debug('%s contains excluded tag %s, skipping.' % (
                                str(trackitemCopy), str(excluded_tag_names)))
                            ignoredTrackItems.add(trackitem)
                            continue
        ...
        for trackitem, trackitemCopy in exportTrackItems:
            ...
            trackItemVersionIndex = self._getTrackItemExportVersionIndex(
                trackitem, versionIndex, findTrackItemExportTag(self._preset, trackitem))
            trackItemVersion = 'v%s' % format(
                int(trackItemVersionIndex), '0%id' % int(versionPadding))
            ...
            for (exportPath, preset) in self._exportTemplate.flatten():
                ...
                taskData = hiero.core.TaskData(preset,
                                               ...
                                               startFrameSource=self._preset.properties()[
                                                   'startFrameSource'],
                                               resolver=resolver,
...
class ShotProcessorPreset(hiero.core.ProcessorPreset):
    ...
    def addCustomResolveEntries(self, resolver):
        ...
        resolver.addResolver(
            kFileHeadKeyword, KeywordTooltips[kFileHeadKeyword], lambda keyword, task: task.filehead())
        resolver.addResolver(
            kFilePathKeyword, KeywordTooltips[kFilePathKeyword], lambda keyword, task: task.filepath())
        ...
        resolver.addResolver(
            '{fileext}', 'Filename extension part of the media being processed', lambda keyword, task: task.fileext())
        resolver.addResolver(
        ...
        resolver.addResolver(
            '{event}', 'EDL event of the track item being processed', lambda keyword, task: task.editId())
        resolver.addResolver('{_nameindex}', 'Index of the shot name in the sequence preceded by an _, for avoiding clashes with shots of the same name',
