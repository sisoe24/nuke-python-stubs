...
import hiero.core
from hiero.core import Keys, Transition, FnResolveTable, log, util
from hiero.core.FnExporterBase import (mediaSourceExportFileHead,
                                       mediaSourceExportReadPath)
...
class EDLExportAudioTrackTask:
    ...
    def taskStep(self):
        ...
        numChannels = int(metadata.value(Keys.kSourceNumAudioChannels)
                          ) if metadata.hasKey(Keys.kSourceNumAudioChannels) else 1
        if numChannels == 1:
        ...
        srcExit = TimeCode.createFromFrames(
            sourceIn + trackItem.sourceDuration(), self._fps, dropFrame)
        syncEntry = TimeCode.createFromFrames(
            timecodeStart + trackItem.timelineIn(), self._fps, dropFrame)
        syncExit = TimeCode.createFromFrames(
            timecodeStart + trackItem.timelineIn() + trackItem.duration(), self._fps, dropFrame)

        editName = self._parent.trackItemEditName(trackItem)

        # Build an edit decision object.
        edit = EditDecision(self._eventIndex, editName, mode, effect,
                            srcEntry, srcExit, syncEntry, syncExit)
...
class EDLExportTrackTask:
    ...
    def createFadeIn(self, trackItem):
        ...
        blankEdit = EditDecision(self._eventIndex, 'BL', mode, EffectField(
            EffectField.CUT, 0), zeroTime, zeroTime, syncEntry, syncEntry)
    ...
    def createTransitionTo(self, trackItem, nextTrackItem):
        ...
        mode = ModeField(False, True, [])
        effect = EffectField(EffectField.CUT, 0)
        fromEdit = EditDecision(self._eventIndex, self._parent.trackItemEditName(
        ...
        effect = EffectField(EffectField.DISSOLVE, transitionDuration)
        toEdit = EditDecision(self._eventIndex, self._parent.trackItemEditName(
            nextTrackItem), mode, effect, toSrcEntry, toSrcExit, toSyncEntry, toSyncExit)
    ...
    def trackItemTimes(self, trackItem):
        ...
        srcExit = TimeCode.createFromFrames(
            sourceIn + trackItem.sourceDuration(), self._fps, self._dropFrame)
        syncEntry = TimeCode.createFromFrames(
            timecodeStart + trackItem.timelineIn(), self._fps, self._dropFrame)
        syncExit = TimeCode.createFromFrames(
            timecodeStart + trackItem.timelineIn() + trackItem.duration(), self._fps, self._dropFrame)
    ...
    def taskStep(self):
        ...
        log.debug('EDLExportTask.taskStep: ' + str(trackItem) + ' in transition: ' +
                  str(inTransition) + ' out transition: ' + str(outTransition))
...
class EDLExportTask(hiero.core.TaskBase):
    ...
    class ShotResolveTable(FnResolveTable.ResolveTable):
        def __init__(self, trackItem, absPath):
            ...
            self.addResolver('{shot}', 'Name of the TrackItem being processed',
                             lambda keyword, trackItem: trackItem.name())
            self.addResolver('{clip}', 'Name of the source Media clip being processed',
                             lambda keyword, trackItem: trackItem.source().name() if trackItem.source() else None)
            self.addResolver('{track}', 'Name of the track being processed', lambda keyword,
                             trackItem: trackItem.parent().name() if trackItem.parent() else None)
            self.addResolver('{sequence}', 'Name of the Sequence being processed', lambda keyword,
                             trackItem: trackItem.parent().parent().name() if trackItem.parent().parent() else None)
            self.addResolver('{event}', 'Event Number of the shot being processed',
                             lambda keyword, trackItem: str(trackItem.eventNumber()))
            self.addResolver('{fps}', 'Frame rate of the Sequence', lambda keyword, trackItem: str(
                trackItem.parent().parent().framerate()) if trackItem.parent().parent() else None)
            self.addResolver('{filename}', "File name part of the TrackItem's Source file.",
                             lambda keyword, trackItem: _filenameFromTrackItem(trackItem, absPath))
            self.addResolver(
                kFileBaseKeyword, KeywordTooltips[kFileBaseKeyword], lambda keyword, trackItem: _filebaseFromTrackItem(trackItem))
            self.addResolver('{fileext}', "File name extension of the TrackItem's Source file.",
                             lambda keyword, trackItem: _fileextFromTrackItem(trackItem))
            self.addResolver(
                kFileHeadKeyword, KeywordTooltips[kFileHeadKeyword], lambda keyword, trackItem: _fileheadFromTrackItem(trackItem))
