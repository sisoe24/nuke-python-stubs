...
class ExternalRenderTaskUI(hiero.ui.TaskUIBase):
    def __init__(self, preset):
        ...
        hiero.ui.TaskUIBase.__init__(
            self, FnExternalRender.ExternalRenderTask, preset, 'External Render')
...
class NukeRenderTaskUI(hiero.ui.RenderTaskUIBase):
    ...
    def populateUI(self, widget, exportTemplate):
        ...
        uiProperty = UIPropertyFactory.create(type(''), key='writeNodeName', value='Write_{ext}', dictionary=self._preset.properties(
        ), label='Write Node Name', tooltip=writeNodeNameToolTip)
    ...
    def _burninEditClicked(self):
        dialog = KnobEditDialog(self._preset.properties(
        )['burninData'], FnExternalRender.NukeRenderTask.burninPropertyData)
        if dialog.exec_():
    ...
    def initUI(self):
        ...
        for datadict in self._properties:
            uiProperty = UIPropertyFactory.create(type(
                datadict['value']), key=datadict['knobName'], value=datadict['value'], dictionary=self._data, label=datadict['label'], tooltip=toolTip)
            self._uiProperties.append(uiProperty)
        ...
        self._standardButtons.button(
            QtWidgets.QDialogButtonBox.RestoreDefaults).clicked.connect(self.reset)
