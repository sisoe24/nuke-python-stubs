...
class LocalNukeEngineRenderTask(hiero.core.TaskBase):
    ...
    def _tryToConnectToOpgraphServer(self):
        try:
            ...
            self._opgraph_socket.connect(
                (NukeEngineOpgraphServerConfig.OPGRAPH_HOST, NukeEngineOpgraphServerConfig.OPGRAPH_PORT))
            return True
        except:
            self._opggraph_socket = None
            return False
    ...
    def _renderOpgraphWithServer(self):
        ...
        # Very simple implementation - the only thing it's expecting is a path to a nk script
        self._opgraph_socket.sendall(self._scriptPath)
    ...
    def startTask(self):
        ...
        try:
            ...
            raise Exception(
                'No Nuke Engine Executable path specified. This can be specified in the ENGINE_PATH environment variable')
        ...
        self._nukeEngineProcess = subprocess.Popen(
            ''.join(engine_args), stdout=self._logFile, stderr=subprocess.STDOUT, shell=True)
    ...
    def parseEngineOut(self):
        ...
        write_regex = re.compile(
            'ComputeVertexI::compute\s+on\s+vertex\s+[0-9_]*(Write)[Container]*')
    ...
    def finishTask(self):
        ...
        if self._report_opgraph_time_as_error:
            self.setError('The opgraph was not rendered on a server and took: %dms to create' %
                          self._opgraph_creation_time)
        ...
        if self._nukeEngineProcess is not None:
            ...
            if returncode is None:
                if platform.system() == 'Windows' and self._usingSingleSocket:
                    # As above
                    subprocess.call(['taskkill', '/F', '/T', '/PID',
                                    str(self._nukeEngineProcess.pid)])
                else:
