...
class NodePropertyWidget(QtWidgets.QWidget):
    ...
    def createProperty(self, label, key, values, tooltip, isVisible, isEnabled):
        ...
        propertyWidget = UIPropertyFactory.create(type(
            values), key=key, value=values, dictionary=self._presetDictionary, label=label, tooltip=tooltip)
    ...
    def enumKnobValues(self, enumKnob):
        ...
        values = [self.enumStringKeyValuePair(
            val) for val in values if self.enumStringKeyValuePair(val)]
        return values
    ...
    def updatePresetValue(self, knob):
        ...
        if knob.visible():
            isInt = knob.getFlag(self.STORE_INTEGER) and not isinstance(
                knob, nuke.Enumeration_Knob)
            self._presetDictionary[knob.name()] = int(
                knob.value()) if isInt else knob.value()
        else:
