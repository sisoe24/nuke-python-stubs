...
def checkPresetRenderOutput(preset):
    ...
    if renderOutputType in kMovieFileTypes:
        QtWidgets.QMessageBox.warning(hiero.ui.mainWindow(
        ), 'Create Comp',  "Cannot create a comp with Write node file type set to '%s'.  Only rendering to image sequences is currently supported." % renderOutputType)
        return False
...
class CreateCompShotTableSortModel(QtCore.QSortFilterProxyModel):
    ...
    def lessThan(self, source_left, source_right):
        ...
        if isEqual:
            isLessThan = source_left.data(
                kModelTrackIndexRole) < source_right.data(kModelTrackIndexRole)
        return isLessThan
    ...
    def createShotsTableWidget(self):
        ...
        for index, item in enumerate(self._trackItems):
            ...
            timelineTimeItem = QtGui.QStandardItem(
                str(item.timelineIn()) + '-' + str(item.timelineOut()))
        ...
        table.setItemDelegateForColumn(
            0, CreateCompShotTableDelegate(QtCore.QSize(16, 16), 10, table))
        table.setItemDelegateForColumn(
            1, CreateCompShotTableDelegate(QtCore.QSize(70, 40), 10, table))
        table.setItemDelegateForColumn(
            2, CreateCompShotTableDelegate(QtCore.QSize(70, 40), 10, table))

        # Set the selection mode and select the first row
        table.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        table.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        table.selectionModel().select(sortModel.index(
            0, 0), QtCore.QItemSelectionModel.SelectCurrent | QtCore.QItemSelectionModel.Rows)
...
class CreateCompMessageBoxErrorHandler(object):
    ...
    def error(self, message):
        ...
        detail = message
        brief = self.getBriefTextFromMessage(
            message) or CreateCompMessageBoxErrorHandler.kDefaultBriefMessage
...
class CreateCompActionBase(BuildTrackFromExportTagAction):
    ...
    def _layoutScripts(self, trackItemData):
        ...
        counter = 0.0
        # Add 1 to the total so the progress doesn't go to 100% in this phase
        total = len(trackItemData) + 1.0
    ...
    def _processExportedTrackItems(self, trackItemData, sequence, project):
        ...
        for trackItem, tag, version in trackItemData:
            if tag:
                compTrackItem = self._findTrackItemByUID(
                    trackItem.sequence(), tag.metadata().value('tag.trackItem'))
                if compTrackItem:
    ...
    def _exportSequence(self, trackItems, masterTrackItem, version,
        ...
        # Copy the preset so we can modify its properties
        preset = hiero.core.taskRegistry.copyPreset(self.shotPreset)
    ...
    def _selectTrackName(self, trackNamePossibilities):
        ...
        label = QtWidgets.QLabel(
            "The selected track items have tags that apply to different target tracks. Please select the target track you'd like the external media track items to be placed onto:")
        ...
        buttonbox.button(QtWidgets.QDialogButtonBox.StandardButton.Ok).setToolTip(
            'Causes the new external media track items to be created on the selected target track.')
    ...
    def _handleMismatchedTracks(self, selectedItems, sequence, project):
        ...
        if foundMismatch:
            ...
            if project.isRestricted():
                QtWidgets.QMessageBox.critical(hiero.ui.mainWindow(
                ), 'HieroPlayer', 'The selected track items have tags that apply to different target tracks. Please re-build the external media track for the selected track items (in Hiero), save the project, and try again.')
                return False
    ...
    def createTrackItemCollisionDialog(self, srcCollisions, track, uniqueTrackName):
        ...
        dialog = TrackItemCollisionDialog(len(srcCollisions), track, uniqueTrackName,
                                          labelText=labelText, enableDeleteButton=False, enableRefreshButton=False)
    ...
    def execute(self, singleNukeScript=None, singleNukeScriptMasterTrackItem=None):
        ...
        if len(self._trackItems) > 1 and not hiero.core.isHieroPlayer():
            ...
            if needsDialog == True:
                dialog = CreateCompForTrackItemsDialog(
                    self._trackItems, self.shotPreset, parent=hiero.ui.mainWindow())
        ...
        if self._singleNukeScript:
            ...
            try:
                ...
                if trackItemData:
                    trackItemsToBuildFrom = self._exportShots(
                        trackItemData, self._effectItems, self._annotations, project)
