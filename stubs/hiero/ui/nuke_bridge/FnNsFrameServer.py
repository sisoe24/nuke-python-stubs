...
if nuke.env['hiero']:
    ...
    def executeInMainThread(callableObject, args): return _executeInMainThread(
        callableObject, *args)
else:
...
frameServer = frameserver.FrameServer(hiero.core.getBundledNukePath(
), hiero.core.getBundledPythonPath(), pathRemapFunc=hiero.core.remapPath)
...
def onError(message):
    ...
    class DetailedResizableMessageBox(QtWidgets.QMessageBox):
        def __init__(self, parent, title, summaryText, detailedText):
            super(DetailedResizableMessageBox, self).__init__(
                QtWidgets.QMessageBox.Critical, title, summaryText, QtWidgets.QMessageBox.Ok, parent)
...
def setBackgroundRenderer(renderer):
    ...
    assert issubclass(renderer.__class__,
                      IBackgroundRenderer), 'Not a subclass of IBackgroundRenderer'
