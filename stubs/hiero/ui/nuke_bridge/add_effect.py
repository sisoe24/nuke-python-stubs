...
def createEffectActions():
    ...
    if not isPlayer:
        ...
        ocioMenu = QMenu('OCIO')
        ocioMenu.setObjectName('foundry.timeline.effect.ocioMenuMain')

        action = QAction(QIcon(gNukeIconPath + '/OCIO.png'), 'OCIO', None)
        action.setObjectName('foundry.timeline.effect.ocioMenuAction')
        action.setToolTip('OCIO')
        action.setMenu(ocioMenu)
        registerAction(action)
    ...
    action = QAction(QIcon(gNukeIconPath + '/OCIO.png'),
                     'OCIO NamedTransform [BETA]', None)
