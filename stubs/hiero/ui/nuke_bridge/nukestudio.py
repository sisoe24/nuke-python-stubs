...
from hiero.ui.nuke_bridge.FnNsFrameServer import (
    stopServer, startServer, workerCount, cancelFrames, onFatalError,
    renderFrames, renderScript, isServerRunning, promptFrameServerKill,
    setBackgroundRenderer, configureFrameServerFromCommandArgs)
...
def mergeMenus():
    ...
    fileMenu.menu().insertSeparator(quitAction)
    insertNukeAction(nukeMenuBar.findItem('File/New Comp...'), fileMenu.menu(),
                     before='foundry.application.quit', context=hiero.ui.kContextComp)
    ...
    insertAction(saveAndReRenderAction, fileMenu.menu(),
                 before='foundry.application.quit', context=hiero.ui.kContextTimeline)
    insertNukeAction(nukeSaveAsItem, fileMenu.menu(), before='foundry.application.quit')
    insertNukeAction(nukeMenuBar.findItem('File/Save New Comp Version'),
                     fileMenu.menu(), before='foundry.application.quit')
    fileMenu.menu().insertSeparator(quitAction)
    insertNukeAction(nukeMenuBar.findItem('File/Insert Comp Nodes...'),
                     fileMenu.menu(), before='foundry.application.quit')
    insertNukeAction(nukeMenuBar.findItem('File/Export Comp Nodes...'),
                     fileMenu.menu(), before='foundry.application.quit')
    insertNukeAction(nukeMenuBar.findItem('File/Comp Script Command...'), fileMenu.menu(),
                     before='foundry.application.quit', context=hiero.ui.kContextComp)
    insertNukeAction(nukeMenuBar.findItem('File/Run Script...'), fileMenu.menu(),
                     before='foundry.application.quit', context=hiero.ui.kContextComp)
    insertNukeAction(nukeMenuBar.findItem('File/Comp Info'), fileMenu.menu(),
                     before='foundry.application.quit', context=hiero.ui.kContextComp)
    fileMenu.menu().insertSeparator(quitAction)
    insertNukeAction(nukeMenuBar.findItem('File/Clear'), fileMenu.menu(),
                     before='foundry.application.quit', context=hiero.ui.kContextComp)
    fileMenu.menu().insertSeparator(quitAction)
    ...
    insertNukeAction(nukeMenuBar.findItem('Workspace/Close Tab'),
                     workspaceMenu, after='foundry.menu.navigation')
...
class OpenInNodeGraphAction(QtWidgets.QAction):
    ...
    def eventHandler(self, event):
        ...
        self.shotSelection = [item for item in event.sender.selection() if (
            isinstance(item, hiero.core.TrackItem) or isinstance(item, hiero.core.BinItem))]
...
class NukeStudioEventHandler:
    def __init__(self):
        ...
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kDoubleClick, hiero.core.events.EventType.kBin), self.onDoubleClick)
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kDoubleClick, hiero.core.events.EventType.kTimeline), self.onDoubleClick)

        # Connect kContextChanged callback for automatically switch application menus.
        hiero.core.events.registerInterest(
            hiero.core.events.EventType.kContextChanged, self.onContextChanged)
            ...
            if lastScriptFile != filePath:  # If the current open script is not the same as the one we're trying to open
                if nuke.executing():
                    ...
                    button = QtWidgets.QMessageBox.warning(
                        hiero.ui.mainWindow(), 'Unable to close', popUpMessage)
                    return
