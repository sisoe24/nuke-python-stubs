...
class HieroState:
    ...
    def _setVersion(self, trackItem, newVersionPath):
        ...
        hiero.core.log.debug(
            'Creating and setting Version; scan successful; should set media / do a rescan of the clip.')
    ...
    def _scanAndBump(self, trackItem, mediaPath, lastMediaPath):
        try:
            ...
            if not self._findNewVersion(lastMediaPath, mediaPath):
                hiero.core.log.debug('Bad Version filename - lastMediaPath:',
                                     lastMediaPath, 'mediaPath:', mediaPath)
                return
    ...
    def _findTrackItem(self, mediaDesc):
            ...
            if not mediaDesc.project_guid() or project.guid() == mediaDesc.project_guid():
                ...
                for sequence in project.sequences():
                    if sequence.guid() == mediaDesc.sequence_guid():
                        ...
                        hiero.core.log.debug(
                            'shot guid and trackitem guid mismatch %s' % (mediaDesc.shot_guid()))
                        return None, None
        ...
        hiero.core.log.debug('Project guid not found [%s not in %s]' % (
            mediaDesc.project_guid(), ', '.join([project.guid() for project in hiero.core.projects()])))
        return None, None
    ...
    def _findTrackItemFromTable(self, mediaDesc):
        ...
        for oldMediaPath in self._trackItems:
            ...
            if mediaDesc.track() and (mediaDesc.track() != self._originalTrackItems[trackItem].parentTrack().name()):
                hiero.core.log.debug('Track name mismatch %s - %s' % (mediaDesc.track(),
                                     self._originalTrackItems[trackItem].parentTrack().name()))
                continue

            if mediaDesc.sequence() and (mediaDesc.sequence() != self._originalTrackItems[trackItem].parentSequence().name()):
                hiero.core.log.debug('Sequence name mismatch %s - %s' % (mediaDesc.sequence(),
                                     self._originalTrackItems[trackItem].parentSequence().name()))
                continue
    ...
    def _findClipsForPath(self, mediaPath):
        # Try to normalise paths with SequenceifyFilename
        mediaPath = hiero.core.util.SequenceifyFilename(mediaPath, False)
        for project in hiero.core.projects():
            for clip in project.clips():
                clipPath = hiero.core.util.SequenceifyFilename(
                    clip.mediaSource().fileinfos()[0].filename(), False)
                # Use startswith because there might be trailing frame number specifications in clipPath if it's an image sequence
                if clipPath.startswith(mediaPath):
                    yield clip
