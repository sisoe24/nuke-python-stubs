...
from hiero.ui import menuBar, findMenuAction
from hiero.core import log, util
            ...
            if rootNode:
                ...
                if ocioconfig.hasRole(inColorspace):
                    ...
                    try:
                        ...
                        print("  Error: inColorspace '" + str(inColorspace) +
...
class FlipbookNuke(flipbooking.FlipbookApplication):
        ...
        if isAppendClipNode:
            ...
            for index in range(nodeToFlipbook.inputs()):
                ...
                if inputFilename is None or inputFilename == '':
                    raise RuntimeError("Cannot run a flipbook on '%s', expected to find a filename for input %d and there was none." % (
                        nodeToFlipbook.fullName(), index))
    ...
    def run(self, filename, frameRanges, views, options):
        if filename is None or filename == '':
            raise RuntimeError("Cannot run a flipbook on '%s', expected to find a filename and there was none." % (
                nodeToFlipbook.fullName(),))
    ...
    def flipbook(self, videoClips, audioClip, first, last, views, options):
        ...
        try:
            ...
            for videoClip, inColorspace in videoClips:
                ...
                log.debug('FlipbookNuke.flipbook clip %s added from %d to %d ' %
                          (videoClip, clipInitTime, sequenceEndTime))
            ...
            # Flipbook sequences are a special case that don't need adding to a bin/project before setting on a player.
            cv.setSequence(newSequence, buffer)
    ...
    def _addFadeInFadeOutTransitions(self, sequence, fadeInTime, fadeOutTime):
        ...
        if len(videoTracks) > 0:
            ...
            if fadeInTime > 0:
                fadeInTransition = hiero.core.Transition.createFadeInTransition(
                    subTrackIn, fadeInTime)
                videoTrack.addTransition(fadeInTransition)

            if fadeOutTime > 0:
                fadeOutTransition = hiero.core.Transition.createFadeOutTransition(
                    subTrackOut, fadeOutTime)
    ...
    def _addDissolveTransition(self, sequence, dissolveTime):
        ...
        if len(videoTracks) > 0 and dissolveTime > 0:
            ...
            for index in range(len(list(videoTrack.items())) - 1):
                ...
                dissolveTransition = hiero.core.Transition.createDissolveTransition(
                    trackItemIn, trackItemOut, tempDissolveTime, tempDissolveTime)
        ...
        if flipbookDialog._useRoi.value():
            ...
            if (roi['r'] - roi['x'] > 0) and (roi['t'] - roi['y'] > 0):
                options['roi'] = bboxToTopLeft(
                    int(nuke.value(nodeToFlipbook.name()+'.actual_format.height')), roi)
...
def _getIntermediatePath(self):
    ...
    if len(self._selectedViews()) > 1 and fileType is not 'exr':
        ...
        flipbookFileNameTemp = '%s_flip_%i.%s.' % (
            self._node.name(), int(time.time()), '%04d') + fileType
    ...
    if not os.path.isdir(util.asUnicode(flipbooktmpdir)):
        raise RuntimeError(
            '%s already exists and is not a directory, please delete before flipbooking again' % flipbooktmpdir)
