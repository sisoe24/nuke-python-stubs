...
def getProjectShotPreset(project):
    ...
    if not name:
        shotPreset = ProjectShotPresetDialog.getPresetFromDialog(
            project, 'Please select a shot export preset:')
        return shotPreset
...
class SendToNukeHandler:
    def __init__(self, hieroState):
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kBin), self.binContextMenuEventHandler)
        hiero.core.events.registerInterest((hiero.core.events.EventType.kShowContextMenu,
                                           hiero.core.events.EventType.kTimeline), self.timelineContextMenuEventHandler)
        hiero.core.events.registerInterest((hiero.core.events.EventType.kShowContextMenu,
                                           hiero.core.events.EventType.kSpreadsheet), self.timelineContextMenuEventHandler)
        ...
        if not hiero.core.isHieroPlayer():
            hiero.core.events.unregisterInterest(
                (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kBin), SendToNuke.sendToNukeAction.eventHandler)
            hiero.core.events.unregisterInterest(
                (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kBin), SendToNuke.add32Action.eventHandler)
            hiero.core.events.unregisterInterest(
                (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kBin), SendToNuke.remove32Action.eventHandler)
                    ...
                    for track in item.videoTracks():
                        for subTrackItems in track.subTrackItems():
                            for subTrackItem in subTrackItems:
                                self._findItems(subTrackItem, clips, sequences,
                                                trackItems, bins, effectItems, annotations)
