...
class RenameShotsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        autoCompleteKeywords = ['{shot}', '{clip}', '{track}',
        ...
        self._resolver.addResolver(
            '{clip}', 'Name of the source Media clip being processed', lambda keyword, task: task.source().name())
        self._resolver.addResolver(
            '{track}', 'Name of the track being processed', lambda keyword, task: task.parent().name())
        self._resolver.addResolver('{sequence}', 'Name of the Sequence being processed',
        ...
        for i in range(0, self._resolver.entryCount()):
            resolverTooltip += '<b>%s</b> - %s<br>' % (
                self._resolver.entryName(i), self._resolver.entryDescription(i))
      ...
      %s""" % resolverTooltip)
        ...
        self._patternField.setToolTip(
            'The pattern to use to rename the selected shots. Only letters, numbers, _ - + and . may be in the name.\nUse one or more # characters to indicate where the number should be placed.')
        ...
        clipNameLayout.addRow('', QtWidgets.QLabel(
            'Rename each shot to its original source Clip name.'))
        ...
        self._buttonbox = QtWidgets.QDialogButtonBox(
            QtWidgets.QDialogButtonBox.RestoreDefaults | QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel)
        self._buttonbox.button(QtWidgets.QDialogButtonBox.Ok).setText('Rename')
        self._buttonbox.accepted.connect(self.accept)
        self._buttonbox.rejected.connect(self.reject)
        self._buttonbox.button(QtWidgets.QDialogButtonBox.RestoreDefaults).clicked.connect(
    ...
    def _updateOkButtonState(self):
        ...
        enableOk = False
        if renameMode == self.kSequentialRename:
            enableOk = '#' in self.pattern() and len(self._startField.text()) > 0 and len(
                self._incField.text()) > 0 and self.increment() > 0
        elif renameMode == self.kFindAndReplace:
...
class RenameTimelineShotsAction(QtWidgets.QAction):
    ...
    def doit(self):
        ...
        if self._dialog.exec_(shots[0].parentSequence(), shots):
            ...
            if not self._dialog.renameAudioTracks():
                ...
                shots = [shot for shot in shots if not isinstance(
                    shot.parent(), hiero.core.AudioTrack)]
            ...
            with project.beginUndo('Shot Rename'):
                ...
                if mode == self._dialog.kSequentialRename:
                    ...
                    if '#' in pattern:
                        ...
                        msgBox.setText(
                            'The pattern must contain at least one # to indicate the position for the shot number.')
                        msgBox.exec_()
                    ...
                    if newName:
                        ...
                        if newName not in self._dialog.searchHistoryList:
                            ...
                            self._dialog.searchHistoryList.pop(
                                self._dialog.searchHistoryList.index(newName))
                            self._dialog.searchHistoryList.append(newName)
                        self._dialog.loadAutocompleter()
                    ...
                    if matchSeq is not None:
                        ...
                        for track in matchSeq.videoTracks():
                            for ti in list(track.items()):
                                ...
                                clip = ti.source()
                                matchShotList.append(
                                    (ti, clip, ti.sourceIn() + clip.timecodeStart()))
                        ...
                        for shot in shots:
                            ...
                            if bestMatchShot is not None:
                                ...
                                hiero.core.log.debug("Match Sequence shot rename didn't find a shot in", matchSeq.name(
                                ), 'that uses a clip named', clip.name(), 'and has overlapping timecode.')
    ...
    def eventHandler(self, event):
        ...
        trackSelection = [item for item in self._selection if isinstance(
