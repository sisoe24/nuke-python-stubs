...
class ScanForVersionsAction(QtWidgets.QAction):
    ...
    def __init__(self):
        ...
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kTimeline), self.eventHandler)
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kSpreadsheet), self.eventHandler)
                ...
                for a in event.menu.actions():
                    if a.text().lower().strip() == 'version':
                        if self._menuActionsAreEnabled(a.menu()):
                            # Insert after 'Version' sub-menu
                            hiero.ui.insertMenuAction(
                                self, a.menu(), before='foundry.project.versionup')
                            break
