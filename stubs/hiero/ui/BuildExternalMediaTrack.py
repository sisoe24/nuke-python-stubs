    ...
    def checkTrackItemCollisionsShowDialog(self, selection, track):
        ...
        for item in selection:
            if isinstance(item, hiero.core.TrackItem):
                ...
                BuildTrack.CheckForTrackItemCollisions(
                    timelineIn, timelineOut, track, False, collidedItems, collidedTransitions)
    ...
    def findTrack(self, name, selection, sequence):
        ...
        while readyToBuild == 0:
            ...
            if not isNewTrack:
                ...
                # Look for collisions
                newSelection, returnedTrack = self.checkTrackItemCollisionsShowDialog(
                    selection, track)
...
class TrackFinderByTag(object):
    ...
    def findTrack(self, name, selection, sequence):
        ...
        readyToBuild = 0
        while readyToBuild == 0:
            track, isNewTrack, lastCheckedTrackIndex = TrackFinderByTag.FindOrCreateTrackFromIndex(
                startItem, sequence, name)
...
class BuildTrack(QtWidgets.QMenu):
    ...
    def eventHandler(self, event):
        ...
        trackSelection = [item for item in selection if isinstance(
    ...
    def CopyTimingFrom(toTrackItem, fromTrackItem, expectedHandles=None):
        ...
        if isinstance(toTrackItem, hiero.core.TrackItem) and isinstance(fromTrackItem, hiero.core.TrackItem):
            ...
            # Debug printing
            hiero.core.log.debug('TrackItem Duration A:%i, B:%i' %
                                 (toTrackItem.duration(), fromTrackItem.duration()))
            hiero.core.log.debug('Clip Duration A:%i, B:%i' %
                                 (toClip.duration(), fromClip.duration()))
            hiero.core.log.debug('Source Duration A:%i, B:%i' %
                                 (toSource.duration(), fromSource.duration()))
            ...
            if toSource.duration() == fromSource.duration():
                ...
                if expectedHandles:
                    ...
                    inTransLength, outTransLength = BuildTrack.CalcTransitionExtraLengths(
                        fromTrackItem)
...
class TrackItemCollisionDialog(QtWidgets.QDialog):
    ...
    def __init__(self, numCollisions, track, uniqueTrackName, labelText=None, parent=None, enableDeleteButton=True, enableRefreshButton=True):
        ...
        if enableDeleteButton:
            self._deleteButton = QtWidgets.QRadioButton(
                'Overwrite the %s colliding items found on the track.' % numCollisions)
        ...
        if enableRefreshButton:
            self._findVersionsButton = QtWidgets.QRadioButton(
                "Refresh the %s colliding items' source clips." % numCollisions)
...
class BuildTrackActionBase(QtWidgets.QAction):
        ...
        if files:
            ...
            if not clip:
                ...
                if self._processorPreset:
                    versionIndex, versionPadding = self._processorPreset.properties(
                    )['versionIndex'], self._processorPreset.properties()['versionPadding']
                    file = file.replace('v*', 'v%s' %
                                        format(versionIndex, '0%id' % int(versionPadding)))
                ...
                try:
                    ...
                    newSource = hiero.core.MediaSource.createOfflineVideoMediaSource(
                        file, expectedStartTime, expectedDuration, sequence.framerate(), startTimecode)
                    ...
                    try:
                        ...
                        newOutputSource = hiero.core.MediaSource.createOfflineVideoMediaSource(
                            filename, expectedStartTime, expectedDuration, sequence.framerate(), startTimecode)
                        outputClip = BuildTrack.FindOrCreateClip(
                            newOutputSource, bin, sequence.project())
                ...
                # the most likely reason for failure is that it's a movie format and rendering isn't finished.
                    ...
                    self._errors.append(
                        'Could not create clip for %s.  Check that the file has finished rendering.' % file)
                    return

            trackItem = self._buildTrackItem(name, clip, originalTrackItem, expectedStartTime,
                                             expectedDuration, expectedStartHandle, expectedEndHandle, expectedOffset)
    ...
    def getTrackItems(self):
        ...
        # Filter out audio
        selection = [item for item in hiero.ui.activeView().selection(
            ...
            for originalTrackItem in selection:
                ...
                if isinstance(originalTrackItem, hiero.core.TrackItem):
                    if isinstance(originalTrackItem.source(), hiero.core.Clip):
                        ...
                        start, duration, starthandle, endhandle, offset = self.getExpectedRange(
                            originalTrackItem)
                        self.buildShotFromFiles(files, originalTrackItem.name(
                        ), sequence, track, bin, originalTrackItem, start, duration, starthandle, endhandle, offset)
                elif isinstance(originalTrackItem, hiero.core.Transition):
                    # Check for colliding transitions
                    BuildTrack.CheckForTransitionCollisions(
                        originalTrackItem, track, collidedTransitions)
...
class BuildTrackFromExportTagDialog(QtWidgets.QDialog):
    ...
    def __init__(self, selection, createCompClips,  parent=None):
        ...
        self._tagListView.setSizePolicy(
        ...
        tagListSelectionModel.select(self._tagListModel.index(
            self._tagListModel.rowCount()-1, 0), QtCore.QItemSelectionModel.Select)
    ...
    def acceptTest(self):
        if self.tagIdentifier() and self.trackName():
            ...
            QtWidgets.QMessageBox.warning(
                self, 'Build Media Track', 'Please set track name and export tag.', QtWidgets.QMessageBox.Ok)
...
class BuildTrackFromExportTagAction(BuildTrackActionBase):
    ...
    def getExpectedRange(self, trackItem):
        ...
        if tag and tag.metadata().hasKey('startframe') and tag.metadata().hasKey('duration'):
            ...
            start, duration, starthandle, endhandle, offset = BuildTrackActionBase.getExpectedRange(
                self, trackItem)
    ...
    def _buildTrackItem(self, name, clip, originalTrackItem, expectedStartTime, expectedDuration, expectedStartHandle, expectedEndHandle, expectedOffset):
        ...
        fullClipExported = (originalMediaSource.duration() ==
                            mediaSource.duration()) and not appliedRetimes
        ...
        # Otherwise try to use the export handles and retime info to determine the correct source in/out
            ...
            isVideoFile = hiero.core.isVideoFileExtension(
                os.path.splitext(mediaSource.fileinfos()[0].filename())[1])
            if isVideoFile:
                expectedStartTime = 0
...
class BuildExternalMediaTrackDialog(QtWidgets.QDialog):
    def __init__(self,  selection,  parent=None):
        ...
        self._exportTemplateViewer = hiero.ui.ExportStructureViewer(
            self._exportTemplate, hiero.ui.ExportStructureViewer.ReadOnly)
        if project:
    ...
    def acceptTest(self):
        ...
        if not validSelection:
            QtWidgets.QMessageBox.warning(self, 'Build External Media Track',
                                          'Please select an entry in the export template and press Build again.', QtWidgets.QMessageBox.Ok)
        elif not self.trackName():
            QtWidgets.QMessageBox.warning(
                self, 'Build External Media Track', 'Please set a track name', QtWidgets.QMessageBox.Ok)
        else:
            self.accept()
    ...
    def presetChanged(self, index):
        ...
        if self._selection:
            project = self.itemProject(self._selection[0])
        presetsDict = dict([(preset.name(), preset) for preset in hiero.core.taskRegistry.localPresets(
        ) + hiero.core.taskRegistry.projectPresets(project)])
...
class BuildExternalMediaTrackAction(BuildTrackActionBase):
    ...
    def getExternalFilePaths(self, trackItem):
        ...
        taskData = hiero.core.TaskData(self._elementPreset, trackItem, self._exportTemplate.exportRootPath(
        ), self._elementPath, 'v*', self._exportTemplate, project=self._project, resolver=self._resolver)
