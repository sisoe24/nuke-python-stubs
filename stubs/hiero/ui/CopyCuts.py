...
class CopyCutsFromTrackAction(QtWidgets.QAction):
    ...
    class CopyCutsFromTrack(QtWidgets.QDialog):
        def __init__(self,  trackItem,  parent=None):
            ...
            self.trackListView.setToolTip(
                'Selected tracks will have cuts applied to them.')
            self.trackListView.setModel(self.trackListModel)

            # Populate track list
            for track in self.currentTracks:
                self._fromTrackDropdown.addItem(
                    self.getIconForTrack(track), track.name(), track.guid())

            self._fromTrackDropdown.setCurrentIndex(
                len(hiero.ui.activeView().sequence().videoTracks()) - 1)
            ...
            self.nameCopyComboBox.setToolTip(
                "Using From Shot Names - will rename newly cut shots with names matching the From shots.\n'None - does not rename the newly cut shots.")
        ...
        def populateFromTracks(self):
            ...
            for track in self.currentTracks:
                ...
                currentFromIndex = self._fromTrackDropdown.itemData(
                    self._fromTrackDropdown.currentIndex())
        ...
        def fromTrackSelectionChanged(self, item):
            currentFromTrack = [track for track in self.sequence if track.guid(
            ) in [self._fromTrackDropdown.itemData(self._fromTrackDropdown.currentIndex())]][0]
        ...
        def trackSelectionChanged(self, item):
            trackName = [track for track in self.sequence if track.guid() in [
                item.data()]][0]
            currentFromTrack = [track for track in self.sequence if track.guid(
            ) in [self._fromTrackDropdown.itemData(self._fromTrackDropdown.currentIndex())]][0]
            ...
            if anySelected:
                for row in range(self.trackListModel.rowCount()):
                    self.trackListModel.itemFromIndex(self.trackListModel.index(
                        row, 0)).setCheckState(QtCore.Qt.Unchecked)
            else:
                for row in range(self.trackListModel.rowCount()):
                    # Get the current track object by grabbing the data (track guid) stored on the checkbox
                    currentTrack = [track for track in self.sequence if track.guid(
    ...
    def doCuts(self):
        ...
        if dialog.exec_():
            ...
            fromTrack = [track for track in sequence if track.guid(
            ) in [dialog._fromTrackDropdown.itemData(dialog._fromTrackDropdown.currentIndex())]][0]
            allCuts = []
            for destinationTrack in dialog.currentTracks:
                if destinationTrack not in dialog.excludedTracks:
                        ...
                        for track in seq:
                            if track == fromTrack:
                                for clip in list(track.items()):
                                    AllTracks[clip.guid()] = list(
                                        range(clip.timelineIn(), clip.timelineOut()+1))
                        return AllTracks
                    ...
                    if copyAll == False:
                        ...
                        cutItems = [item for item in selection if isinstance(
                            item, hiero.core.TrackItem) and not isinstance(item, hiero.core.Transition)]
                        toClips = getTrackCuts(sequence, toTrack)

                        selectedCuts = {}
                        for clip in cutItems:
                            selectedCuts[clip.name()] = list(
                                range(clip.timelineIn(), clip.timelineOut() + 1))
