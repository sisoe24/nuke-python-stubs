...
class LocaliseMenu:
    ...
    def __init__(self):
        ...
        self._localiseBinContentsAction = createMenuAction(
            'Bin', self.localiseBinSelection, icon='icons:Bin.png')
        self._localiseBinSequenceAction = createMenuAction(
            'Sequence', self.localiseBinSelection, icon='icons:TimelineStroked.png')

        # These actions are added to the right-click menu of the Timeline/Spreadsheet View
        self._localiseSequenceSelectionAction = createMenuAction(
            'Sequence', self.localiseTimelineSpreadsheetSequence, icon='icons:TimelineStroked.png')
        self._localiseTrackSelectionAction = createMenuAction(
            'Track', self.localiseTimelineSpreadsheetSelection, icon='icons:Tracks.png')
    ...
    def binViewEventHandler(self, event):
        ...
        sequenceSelection = [item.activeItem() for item in binItemSelection if isinstance(
            item.activeItem(), hiero.core.Sequence)]
        ...
        if len(self._selection) > 0:
            ...
            self._localiseBinContentsAction.setEnabled(
                localisationEnabled and len(binSelection) >= 1)
            self._localiseBinSequenceAction.setEnabled(
                localisationEnabled and len(sequenceSelection) >= 1)

            # Insert the Localise menu
            insertMenuAction(self._localiseMenu.menuAction(), event.menu,
                             after='foundry.timeline.clipMenu')
        return
    ...
    def timelineSpreadsheetEventHandler(self, event):
        ...
        if len(self._selection) > 0:
            ...
            localisationEnabled = _localisationEnabled()
            self._localiseShotSelectionAction.setEnabled(
                localisationEnabled and len(shotSelection) >= 1)
            self._localiseTrackSelectionAction.setEnabled(
