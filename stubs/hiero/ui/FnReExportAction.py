...
def findOriginalTrackItemAndTag(trackItem):
    ...
    for tag in reversed(trackItem.tags()):
        if tag.metadata().hasKey('tag.originaltrackitem'):
            originalTrackItem = findTrackItemByUID(
                trackItem.sequence(), tag.metadata().value('tag.originaltrackitem'))
...
def findNewVersionIndex(path):
    ...
    try:
        ...
        response = QMessageBox.question(hiero.ui.mainWindow(
        ), 'Re-Export', 'No versions in export path.  Do you want to overwrite?', QMessageBox.Yes | QMessageBox.No)
...
class ReExportAction(QAction):
    ...
    def reExportItem(self, item, tag, newVersionIndex, allTrackItems, enableAnnotations):
        ...
        preset = hiero.core.taskRegistry.findPresetInProjectExportHistory(
            project, tag.metadata()['tag.presetid'])
...
class ReExportEventListener(object):
    ...
    def __init__(self):
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kTimeline), self.onContextMenuEvent)
