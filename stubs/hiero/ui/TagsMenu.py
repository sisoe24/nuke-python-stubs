...
from hiero.ui import findMenuAction, createMenuAction, insertMenuAction
...
class TagsMenu:
    ...
    def __init__(self):
        ...
        self._addTagToCurrentViewerFrame = createMenuAction(
            'Tag this Frame', self.addTagCurrentViewerFrame, icon='icons:TagTiny.png')
        self._addTagToCurrentViewerShot = createMenuAction(
            'Tag this Shot', self.addTagToCurrentViewerShot, icon='icons:Mask.png')
        self._addTagToCurrentViewerClipOrSequence = createMenuAction(
            'Tag whole Sequence', self.addTagToCurrentViewerSequence, icon='icons:TimelineStroked.png')
        self._clearSequenceTags = createMenuAction(
            'Clear Frame Tags', self.clearCurrentViewerTags, icon='icons:TagBad.png')

        # Action for the Bin view to act upon a selection of BinItems
        self._addTagToBinItemSelection = createMenuAction(
            'Tag Selection', self.addTagToBinItemSelection, icon='icons:Add.png')

        # Actions for the Timeline/Spreadsheet View
        self._addTagToShotSelection = createMenuAction(
            'Tag Shot Selection', self.addTagToShotSelection, icon='icons:Mask.png')
        self._addTagToTrackFromSelection = createMenuAction(
            'Tag Tracks', self.addTagToTrackFromSelection, icon='icons:Tracks.png')
        self._addTagToSequenceFromSelection = createMenuAction(
            'Tag this Sequence', self.addTagToSequenceFromSelection, icon='icons:TimelineStroked.png')
        ...
        hiero.core.events.registerInterest(
            'kShowContextMenu/kSpreadsheet', self.timelineSpreadsheetEventHandler)
    ...
    def addTagToActiveViewItems(self):
        ...
        if isinstance(currentView, hiero.ui.BinView):
            binItems = [bi.activeItem() for bi in currentSelection if (
                isinstance(bi, hiero.core.BinItem) and hasattr(bi, 'activeItem'))]
            ...
            shotSelection = [item for item in currentSelection if isinstance(
                item, (hiero.core.TrackItem))]
            trackSelection = [item for item in currentSelection if isinstance(
    ...
    def addTagToBinItemSelection(self):
        ...
        binItems = [bi.activeItem() for bi in self._selection if (
            isinstance(bi, hiero.core.BinItem) and hasattr(bi, 'activeItem'))]
        # Check we have a valid selection of BinItems...
        if len(binItems) < 1:
            return
    ...
    def addTagToTrackFromSelection(self):
        ...
        trackSelection = [track for track in self._selection if isinstance(
            track, (hiero.core.VideoTrack, hiero.core.AudioTrack))]
    ...
    def addTagToSequenceFromSelection(self):
        ...
        selection = [item for item in self._selection if isinstance(
            item, (hiero.core.TrackItem, hiero.core.AudioTrack, hiero.core.VideoTrack))]
        ...
        if self.currentClipSequence == None:
            ...
            if not currentShot:
                QtWidgets.QMessageBox.warning(
                    None, 'Add Shot Tag', 'All Shots at this frame are Offline / Disabled\nNot adding Shot Tag.', QtWidgets.QMessageBox.Ok)
                return
            else:
    ...
    def timelineSpreadsheetEventHandler(self, event):
        ...
        trackSelection = [item for item in s if isinstance(
            item, (hiero.core.VideoTrack, hiero.core.AudioTrack))]
