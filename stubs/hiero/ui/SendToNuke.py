...
from hiero.core.nuke import Script, ScriptWriter, PythonScriptWriter
...
class SendToNukeDialog(QtWidgets.QDialog):
    ...
    def __init__(self, enableSoftTrims, clipFps, settingsPrefix, force32Mode=False, parent=None, windowTitle='Send To Nuke'):
        ...
        self._useSoftTrims.setToolTip(
            "Tick this if you'd like the soft trims to be used for the frame range sent to Nuke.\nThis option is only enabled if the selected clips have soft trims enabled.")
        ...
        self._ingestButton.setToolTip(
            "Tick this field if you'd like the output from the Nuke script to be ingested back into the selected bin.")
        ...
        self._retimeFpsBox.setCurrentIndex(self._loadComboBoxSetting(
            settings, self.kSendToNukeRetimeFpsKey, self._retimeFpsBox))
        self._retimeBox.setCurrentIndex(self._loadComboBoxSetting(
            settings, self.kSendToNukeRetimeSettingKey, self._retimeBox))
    ...
    def saveSettings(self):
        ...
        settings.setValue(self.kSendToNukeRetimeFpsKey,
                          self._retimeFpsBox.itemText(self._retimeFpsBox.currentIndex()))
        ...
        class SpecialTranscodeExporterUI(hiero.exporters.FnExternalRenderUI.NukeRenderTaskUI):
            def __init__(self, dialog):
                hiero.exporters.FnExternalRenderUI.NukeRenderTaskUI.__init__(
                    self, hiero.exporters.FnExternalRender.NukeRenderPreset('SendToNukePreset', presetProperties))
                self._dialog = dialog

            def codecComboBoxChanged(self, value):
                # call the base class
                hiero.exporters.FnExternalRenderUI.NukeRenderTaskUI.codecComboBoxChanged(
                    self, value)
    ...
    def addWriteNode(self, outputPath, inputNode, framerate, projectsettings):
        import hiero.exporters.FnExternalRender
        if self._ui != None:
            writeNode = hiero.exporters.FnExternalRender.createWriteNode(
                self, outputPath, self._ui._preset, inputNode=inputNode, framerate=framerate, projectsettings=projectsettings)
        else:
...
class SendToNukeAction(QtWidgets.QAction):
    ...
    def doit(self, force32Mode=False):
        ...
        d = SendToNukeDialog(enableSoftTrims, clipfps, self._settingsPrefix,
                             force32Mode, windowTitle=self._dialogTitle, parent=hiero.ui.mainWindow())
        if d.exec_() == QtWidgets.QDialog.Accepted:
            ...
            for clip in clips:
                ...
                metadataNode = MetadataNode(
                    metadatavalues=[('hiero/project', clip.project().name())])
                ...
                if fpsForNewSource:
                    ...
                    if fpsForNewSource != clip.framerate():
                        ...
                        (hh, mm, ss, ff) = Timecode.framesToHMSF(
                            timecodeStart, clip.framerate(), clip.dropFrame())
                        timecodeStart = Timecode.HMSFToFrames(
                            fpsForNewSource, clip.dropFrame(), hh, mm, ss, ff)
                    ...
                    w.addNode(AddTimeCodeNode(timecodeStart=timecodeStart, fps=fpsForNewSource,
                              dropFrames=clip.dropFrame(), frame=startFrameWrite))
                    ...
                    metadataNode.addMetadata(
                        [('input/frame_rate', round(fpsForNewSource.toFloat(), NUM_DECIMALS))])
