...
def sgPlaylistFind(playlistname, projectname):
    ...
    fields = ['playlist.Playlist.code', 'sg_sort_order', 'version.Version.code',
              'version.Version.user', 'version.Version.entity']
    fields = ['code', 'version']
...
def sgPlaylistContent(playlistId):
    ...
    fields = ['playlist.Playlist.code', 'version.Version.code',
              'version.Version.entity', 'version.Version.sg_path_to_frames']
...
def sgShotCreate(shotcode, projectname, sgShotDescription='NukeStudio created shot'):
    ...
    if sgShotFind(shotcode, projectname):
        return sgShotFind(shotcode, projectname)
...
def sgTaskCreate(shotcode, projectname, sgTaskName):
    ...
    if sgTaskFind(shotcode, projectname, sgTaskName):
        return sgTaskFind(shotcode, projectname, sgTaskName)
...
def sgVersionCreate(shotcode, projectname, sgTaskName, moviePath, sgVersion, sgPathFrames, sgVersionDescription='NukeStudio created version'):
    ...
    if not sgVersionFind(shotcode, projectname, sgTaskName, sgVersion):
        ...
        # , field_name="sg_latest_quicktime", display_name="Latest QT")
        moviePathId = sg.upload(
            'Version', versionId['id'], moviePath, field_name='sg_uploaded_movie')
...
def qtFromFrames(framePath, rate=24):
    ...
    start_number = 1001
    cmds = [ffmpeg, '-y', '-r', str(rate), '-start_number', str(start_number),
            '-i', framePath, '-c:v', 'libx264', '-crf', '23', moviePath]
...
def selClipsToSg():
    ...
    if seq:
        ...
        if len(te.selection()) == 0:
            raise ValueError('*** No clips selected on the track ***')
        else:
            for shot in te.selection():
                pathFrames = shot.source().mediaSource().metadata()[
                    'foundry.source.path'].split(' ')[0]
                clipShot = shot.name().split('_')[0]
                clipTask = shot.name().split('_')[1]
                a = shot.currentVersion().name()
                clipVersion = int(re.search('_v(\d{2,5})', a).group(1))

                print('==> Working on shot: %s - task: %s and version: v%03d' %
                      (clipShot, clipTask, clipVersion))
                ...
                # SG version creation
                curVersion = sgVersionCreate(
                    clipShot, projectName, clipTask, pathMovie, clipVersion, sgPathFrames, 'NukeStudio version submission')
    ...
    if len(te.selection()) == 0:
        ...
        for shot in te.selection():
            ...
            if sgTS[0]['sg_status_list'] != 'wtg':
                newTag = hiero.core.findProjectTags(
                    tagsProj, sgTaskStatusDict[sgTS[0]['sg_status_list']])[0]
...
def sgPlaylistToTrack(playlistname, projectname):
    ...
    # find the most recent loaded project - not ideal but adequate
    ...
    if not playlist:
        raise ValueError(
            "*** Couldn't find a SG Playlist using that Playlist name and Project name combo.")

    if not seq:
        raise ValueError(
            "*** Couldn't find an active Hiero/NukeStudio Sequence.  Please create or select one.")
...
def menu_sgPlaylistToTrack():
    playlistname = nuke.getInput(
        'Enter Shotgun playlist name to pull to bottom video track:', 'Existing Playlist')
