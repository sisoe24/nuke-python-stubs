...
class CustomTranscode(FnTranscodeExporter.TranscodeExporter):
    ...
    def startTask(self):
        ...
        if isinstance(self._item, hiero.core.Sequence):
            ...
            platformUsesQuicktime = sys.platform.startswith(
                'win32') or sys.platform.startswith('darwin')
            ...
            if includeAudio and isQuicktime and platformUsesQuicktime:
                ...
                if audioTracks:
                    ...
                    audioTrackItems = list(audioTrack.items())
                    ...
                    if audioTrackItems:
                        ...
                        firstSrcFile, firstSrcExt = self.sourceFileFromTrackItem(
                            firstAudioTrackItem)
    ...
    def taskStep(self):
        # The parent implementation of taskstep
        ...
        #  - Parses the output over frame until complete

        return FnTranscodeExporter.TranscodeExporter.taskStep(self)

        ###############################################
        # Alternative implementation for a render farm
        #  - Calls self.writeScript() which writes the script to the path in self._scriptfile
        #  - Copy the script somewhere e.g. shutil.copyfile(self._scriptfile, "/mnt/somewhere/over/the/rainbow/")
        #  - Poll for completion
        #  - Return True whilst incomplete and Return False when no more updates required
