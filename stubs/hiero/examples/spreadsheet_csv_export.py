...
def getSrcIn(trackItem):
    ...
    srcIn = hiero.core.Timecode.timeToString(
        clipstartTimeCode+trackItem.sourceIn(), fps, hiero.core.Timecode.kDisplayTimecode)
    return srcIn
...
def getSrcOut(trackItem):
    ...
    clip = trackItem.source()
    clipstartTimeCode = clip.timecodeStart()
    srcOut = hiero.core.Timecode.timeToString(
        clipstartTimeCode+trackItem.sourceOut()+timecodePrefCheck(), fps, hiero.core.Timecode.kDisplayTimecode)
    return srcOut
...
def getDstIn(trackItem):
    ...
    fps = seq.framerate()
    dstIn = hiero.core.Timecode.timeToString(
        tStart+trackItem.timelineIn(), fps, hiero.core.Timecode.kDisplayTimecode)
    return dstIn
...
def getDstOut(trackItem):
    ...
    tStart = seq.timecodeStart()
    fps = seq.framerate()
    dstOut = hiero.core.Timecode.timeToString(
        tStart+trackItem.timelineOut()+timecodePrefCheck(), fps, hiero.core.Timecode.kDisplayTimecode)
    return dstOut
...
def writeCSVFromSequence(seq):
    ...
    savePath, filter = QFileDialog.getSaveFileName(
        None, caption='Save CSV As...', dir=csvSavePath, filter='*.csv')
    ...
    csvHeader = ['Event', 'Status', 'Shot Name', 'Reel',  'Track', 'Speed', 'Src In',
                 'Src Out', 'Src Duration', 'Dst In', 'Dst Out', 'Dst Duration', 'Clip', 'Clip Media']

    # Get a CSV writer object
    csvWriter = csv.writer(open(savePath, 'w'), delimiter=',',
