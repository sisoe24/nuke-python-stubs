...
class NewSequenceFromSelectionAction(QtWidgets.QAction):
    ...
    class NewSequenceFromSelection(QtWidgets.QDialog):
        ...
        def __init__(self,  trackItem,  parent=None):
            ...
            self.timecodeOptionsLabel.setToolTip(
                'Choose how to handle clips with no inherent timecode.')
            self.timecodeOptionsLabel.setEnabled(
            ...
            self.timecodeOptions.setEnabled(
                self.useTimecode.isChecked() & self.radioSingle.isChecked())
            ...
            self.formatChooser.setToolTip(
                'Choose the format for a New Sequence created from Multiple Selections.')
            ...
            self.fpsChooser.setToolTip(
                'Choose the framerate for a New Sequence created from Multiple Selections.')
            ...
            self.spacerItem2 = QtWidgets.QSpacerItem(
                175, 20, QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
            self.horizontalLayout2.addItem(self.spacerItem2)
            self.groupLayout.setLayout(
                8, QtWidgets.QFormLayout.SpanningRole, self.horizontalLayout2)
    ...
    def __init__(self):
        ...
        hiero.core.events.registerInterest(
            (hiero.core.events.EventType.kShowContextMenu, hiero.core.events.EventType.kBin), self.eventHandler)
        hiero.core.events.registerInterest(
            hiero.core.events.EventType.kSelectionChanged, self.eventHandler)
    ...
    def newStereoSequence(self, leftClip, rightClip, baseName, useTimecode):
        ...
        leftTag = hiero.core.findProjectTags(hiero.core.projects(
            hiero.core.Project.kStartupProjects)[0], 'Left')
        rightTag = hiero.core.findProjectTags(hiero.core.projects(
            hiero.core.Project.kStartupProjects)[0], 'Right')
    ...
    def convertStartTimecode(self, start, sourcerate, destrate):
        ...
        destFrames = hiero.core.Timecode.HMSFToFrames(destrate, False, int(
            startHMSF[0]), int(startHMSF[1]), int(startHMSF[2]), int(startHMSF[3]))
        return destFrames
    ...
    def clipInfo(self, clip, fps):
        ...
        if clip.activeItem().mediaSource().hasVideo():
            start = self.convertStartTimecode(clip.activeItem().timecodeStart(
            ), clip.activeItem().framerate().toInt(), fps.toInt())
            ...
            if len(tracks[trackindex]) > 0:
                ...
                if not clipexists:
                    ...
                    if overlap:
                        ...
                        self.buildTracksFromTimecode([clip], siftedselection, fps, tracks, trackindex=trackindex+1,
                                                     ignoreZeroStart=ignoreZeroStart, singleTrackZero=singleTrackZero)
                    else:
                ...
                if dialog.radioSingle.isChecked() == True:
                    ...
                    if dialog.sequenceLayers.isChecked() == False:
                        ...
                        for item in selection:
                            ...
                            if clip.mediaSource().hasVideo():
                                for i in range(clip.numVideoTracks()):
                                    newVideoTrack = sequence.addTrack(
                                        hiero.core.VideoTrack('Video ' + str(k+1)))
                                    try:
                            ...
                            if clip.mediaSource().hasAudio() and dialog.includeAudio.isChecked():
                                ...
                                for i in range(clip.numAudioTracks()):
                                    newAudioTrack = sequence.addTrack(
                                        hiero.core.AudioTrack('Audio ' + str(j+i+1)))
                                    audioClip = newAudioTrack.addTrackItem(clip, i, 0)
                                    linkedItems.append(audioClip)
                                    if videoClip:
                                        audioClip.link(videoClip)
                    ...
                    if dialog.sequenceLayers.isChecked() == True:
                        if not dialog.useTimecode.isChecked():
                            ...
                            if clip.mediaSource().hasVideo():
                                if clip.mediaSource().metadata()['foundry.source.framerate']:
                                    fps = hiero.core.TimeBase(clip.mediaSource().metadata()[
                                                              'foundry.source.framerate'])
                                else:
                                    fps = clip.framerate()
                                sequence.setFramerate(newFPS)
                            ...
                            if dialog.ignoreZeroStart:
                                cliptracks, zeroStartClips = self.buildTracksFromTimecode(
                                    selection, siftedselection, newFPS, tracks, 0, True)
                            elif not dialog.ignoreZeroStart and dialog.singleTrackZero:
                                cliptracks, zeroStartClips = self.buildTracksFromTimecode(
                                    selection, siftedselection, newFPS, tracks, 0, False, True)
                            else:
                                cliptracks, zeroStartClips = self.buildTracksFromTimecode(
                                    selection, siftedselection, newFPS, tracks, 0, False)
                            if cliptracks:
                                ...
                                for trackindex in range(len(cliptracks)):
                                    sequence.addTrack(hiero.core.VideoTrack(
                                        'Video %s' % (trackindex + 1)))
                                    ...
                                    for item in cliptracks[trackindex]:
                                        ...
                                        if clip.mediaSource().hasVideo():
                                            for i in range(clip.numVideoTracks()):
                                                ...
                                                try:
                                                    videoClip = sequence.videoTrack(
                                                        trackindex).addTrackItem(clip, start)
                                                except:
                                                    print('Failed to add clip')
                                        else:
                                            videoClip = None
                                        ...
                                        if clip.mediaSource().hasAudio() and dialog.includeAudio.isChecked():
                                            ...
                                            for i in range(clip.numAudioTracks()):
                                                ...
                                                try:
                                                    newAudioTrack = sequence.audioTrack(
                                                        trackindex + i)
                                                except:
                                                    newAudioTrack = sequence.addTrack(
                                                        hiero.core.AudioTrack('Audio %s' % (trackindex + i + 1)))
                                                audioClip = newAudioTrack.addTrackItem(
                                                    clip, i, start)
                                                linkedItems.append(audioClip)
                                                if videoClip:
                                                    audioClip.link(videoClip)
                            ...
                            if siftedselection and dialog.singleTrackZero:
                                ...
                                for selection in siftedselection:
                                    ...
                                    self.addClipToSequence(
                                        sequence, clip, start, trackindex, False, False)
                                    start += clip.duration()
                            bin.addItem(hiero.core.BinItem(sequence))
                    ...
                    for pair in stereoPairs:
                        ...
                        if len(pair) == 2:
                            ...
                            try:
                                newName = leftrx.match(leftClip.name()).groups()[0]
                                sequence = self.newStereoSequence(
                                    leftClip, rightClip, newName, dialog.useTimecode.isChecked())
                                bin = leftClip.binItem().parentBin()
                                if sequence:
                                    bin.addItem(hiero.core.BinItem(sequence))
                                    sequence.editFinished()
