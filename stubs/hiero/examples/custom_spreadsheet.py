...
class CustomSpreadsheetColumns(QtCore.QObject):
    ...
    standardColourSpaces = ['linear', 'sRGB', 'rec709', 'Cineon',
                            'Gamma1.8', 'Gamma2.2', 'Panalog', 'REDLog', 'ViperLog']
    arriColourSpaces = ['Video - Rec709', 'LogC - Camera Native',
                        'Video - P3-DCI', 'ACES', 'LogC - Film', 'LogC - Wide Gamut']
    r3dColourSpaces = ['Half Float Linear', 'Rec709', 'REDspace', 'REDlog', 'PDlog685', 'PDlog985',
                       'CustomPDlog', 'REDgamma', 'SRGB', 'REDlogFilm', 'REDgamma2', 'REDgamma3', 'REDgamma4']
    ...
    def paintCell(self, row, column, item, painter, option):
        ...
        if currentColumn['name'] == 'Tags':
            ...
            iconSize = 20
            r = QtCore.QRect(option.rect.x(), option.rect.y() +
                             (option.rect.height()-iconSize)/2, iconSize, iconSize)
            tags = item.tags()
            if len(tags) > 0:
                painter.save()
        ...
        if currentColumn['name'] == 'Thumbnail':
            imageView = None
            pen = QtGui.QPen()
            r = QtCore.QRect(option.rect.x()+2, (option.rect.y() +
                             (option.rect.height()-46)/2), 85, 46)
            if not item.source().mediaSource().isMediaPresent():
    ...
    def createEditor(self, row, column, item, view):
        ...
        if currentColumn['name'] == 'Shot Status':
            cb = QtWidgets.QComboBox()
            cb.addItem('')
            for key in list(gStatusTags.keys()):
    ...
    def colourspaceChanged(self, index):
        ...
        with project.beginUndo('Set Colourspace'):
            items = [item for item in selection if (
                item.mediaType() == hiero.core.TrackItem.MediaType.kVideo)]
...
def _setStatus(self, status):
    ...
    if not status in list(gStatusTags.keys()):
...
class SetStatusMenu(QtWidgets.QMenu):
    ...
    def __init__(self):
        QtWidgets.QMenu.__init__(self, 'Set Status', None)
    ...
    def createStatusMenuActions(self):
        ...
        for status in self.statuses:
            self.menuActions += [titleStringTriggeredAction(
                status, self.setStatusFromMenuSelection, icon=gStatusTags[status])]

    def setStatusFromMenuSelection(self, menuSelectionStatus):
        selectedShots = [item for item in self._selection if (
            isinstance(item, hiero.core.TrackItem))]
        selectedTracks = [item for item in self._selection if (
            isinstance(item, (hiero.core.VideoTrack, hiero.core.AudioTrack)))]

        # If we have a Track Header Selection, no shots could be selected, so create shotSelection list
        if len(selectedTracks) >= 1:
            for track in selectedTracks:
                selectedShots += [item for item in list(track.items())
...
gArtistList = [{'artistName': 'John Smith', 'artistIcon': 'icons:TagActor.png', 'artistDepartment': '3D', 'artistID': 0},
               {'artistName': 'Savlvador Dali', 'artistIcon': 'icons:TagActor.png',
                   'artistDepartment': 'Roto', 'artistID': 1},
               {'artistName': 'Leonardo Da Vinci', 'artistIcon': 'icons:TagActor.png',
                   'artistDepartment': 'Paint', 'artistID': 2},
               {'artistName': 'Claude Monet', 'artistIcon': 'icons:TagActor.png',
                   'artistDepartment': 'Comp', 'artistID': 3},
...
class AssignArtistMenu(QtWidgets.QMenu):
    ...
    def createAssignArtistMenuActions(self):
        ...
        for artist in self.artists:
            self.menuActions += [titleStringTriggeredAction(
                artist['artistName'], self.setArtistFromMenuSelection, icon=artist['artistIcon'])]
