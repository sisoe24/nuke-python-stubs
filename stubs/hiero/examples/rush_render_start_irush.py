...
class RushRenderTask(hiero.core.TaskBase):
    ...
    def sendToRush(self, scriptPath, first, last, job_title):
        ...
        cmd = rushFile + ' -field NukeScript ' + scriptPath + ' -field Frames ' + \
            str(first) + '-' + str(last) + ' -field JobTitle ' + \
            job_title + ' -field JobDoneCommand ' + self._jobDoneCmd
        self.rushProcess = subprocess.Popen(
            cmd, shell=True, stdout=self._logFile, stderr=subprocess.PIPE)
