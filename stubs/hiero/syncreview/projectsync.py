...
messages.defineMessageType('ProjectPush', ('guid', str), ('name', str),
                           ('filename', str), ('content', messages.Compressed))
    ...
    def _onClientsUpdated(self):
        ...
        if loadingCount == 0:
            ...
            if not self._progressTask:
                self._progressTask = ProgressTask(
                    'Users syncing', 100, ProgressTask.kModal | ProgressTask.kShowTimeToGo)
            if loadingCount == 1:
                self._progressTask.setMessage(
                    '{} user is currently syncing...'.format(loadingCount))
            else:
                self._progressTask.setMessage(
                    '{} users are currently syncing...'.format(loadingCount))
