...
messages.defineMessageType('AnnotationAdd', ('sequenceGuid', str),
...
def _findAnnotation(sequenceGuid, annotationGuid):
    ...
    annotation = next((i for i in itertools.chain(*track.subTrackItems())
                      if i.guid() == annotationGuid), None)
    if not annotation:
...
class SyncAnnotationsTool(SyncTool):
    ...
    def __init__(self, messageDispatcher):
        ...
        self.messageDispatcher._registerCallback(
            messages.AnnotationChange, self.onRemoteAnnotationChanged)
        self.messageDispatcher._registerCallback(
            messages.AnnotationRemove, self.onRemoteAnnotationRemoved)
