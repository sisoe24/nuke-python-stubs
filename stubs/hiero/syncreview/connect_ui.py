...
from PySide2.QtWidgets import (QAction, QDialog, QSpinBox, QCheckBox,
                               QLineEdit, QShortcut, QFormLayout, QMessageBox,
                               QPushButton, QInputDialog, QDialogButtonBox)

from . import config, syncreviewstatuspanel
...
class ClientConnectUI(QObject):
    ...
    def onConnectStateChanged(self, newState, oldState):
        clientDisconnected = (newState == ConnectionState.DISCONNECTED) and (
            oldState == ConnectionState.CLIENT_CONNECTED)
...
class MenuBuilder(QObject):
    ...
    def createMenus(self):
        ...
        self.hostSessionAction = hiero.ui.createMenuAction(
            'Host Sync Session...', self._hostUI.startServer, path='foundry.application.hostSession')
        self.endSessionAction = hiero.ui.createMenuAction(
            'End Sync Session', self._hostUI.stopServer, path='foundry.application.endSession')
        self.connectSessionAction = hiero.ui.createMenuAction(
            'Connect to Sync Session...', self._clientUI.connectClient, path='foundry.application.connectSession')
        self.disconnectSessionAction = hiero.ui.createMenuAction(
            'Disconnect from Sync Session', self._clientUI.disconnectClient, path='foundry.application.disconnectSession')
