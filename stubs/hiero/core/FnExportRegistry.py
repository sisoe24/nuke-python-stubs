...
import _nuke
import _fnpython
from hiero.core import (Format, TaskBase, FolderTask, FnFloatRange,
                        TaskPresetBase, FolderTaskPreset, IExporterRegistry,
                        log, isNC, util, isIndie, project)
...
class TaskRegistry (IExporterRegistry):
    ...
    def projectUnloaded(self, project):
        ...
        for preset in presets:
            try:
                ...
                log.error("Couldn't delete preset %s from list %s" %
                          (preset.name(), ', '.join(presets)))
                log.error(str(e))
    ...
    def loadPresets(self, path):
        ...
        processorPresetResult = self._loadPresets(
            os.path.join(path, 'Processors'), self._processorPresets)
    ...
    def presetFromXml(self, xml, register=True):
        ...
        taskTypeDict = {}
        taskTypeDict.update(dict((classBasename(value), self._presets[key]) for (
            key, value) in list(self._tasks.items())))
        taskTypeDict.update(dict((classBasename(value), self._presets[key]) for (
            key, value) in list(self._processors.items())))
    ...
    def savePresets(self, path):
        ...
        try:
            ...
            processorPresetResult = self._savePresets(os.path.join(
                path, 'Processors'), self._processorPresets, self._processors)
    ...
    def _savePresets(self, path, presetlist, dictionary):
        ...
        for preset in presetlist:
            ...
            if savePath:
                newPath = os.path.join(preset.savePath(), classBasename(
    ...
    def _presetToXml(self, preset):
        ...
        if issubclass(type(preset), TaskPresetBase):
            ...
            root = etree.Element('root', tasktype=classBasename(
                preset.parentType()), presetname=preset.name())
    ...
    def discardPresetChanges(self, project):
        ...
        self._processorPresets = [
            preset for preset in self._processorPresets if preset.project() not in (project, None)]
    ...
    def _getPresetsFromList(self, presetsList, projectToFilter):
        ...
        presets = [preset for preset in presetsList if preset.project(
        ) == projectToFilter and not preset.markedForDeletion()]
        return presets
    ...
    def localPresetsChanged(self):
        ...
        storedLocalPresets = list(sorted(self._getPresetsFromList(
            self._storedPresets, None), key=lambda preset: preset.name()))
    ...
    def projectPresetsChanged(self, project):
        ...
        storedProjectPresets = list(sorted(self._getPresetsFromList(
            self._storedPresets, project), key=lambda preset: preset.name()))
        projectPresets = list(sorted(self.projectPresets(
            project), key=lambda preset: preset.name()))
    ...
    def _checkToScaleForSequence(self, presetName, toScaleOptions, itemName, seqFormat):
        ...
        for toScale in toScaleOptions:
            try:
                Format(seqFormat.width() * toScale, seqFormat.height() *
                       toScale, seqFormat.pixelAspect(), seqFormat.name())
            except ValueError as e:
                raise ValueError("Unable to export '%s' because Preset '%s' is scalling it to an invalid output resolution.\n%s" % (
                    itemName, presetName, e.message))
    ...
    def createProcessor(self, preset, submissionName=None, synchronous=False):
        ...
        if not submission:
            raise RuntimeError('Submission class not found for name %s, valid names are: %s' % (
                submissionName, ', '.join(self.submissionNames())))
