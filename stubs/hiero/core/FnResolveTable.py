...
class ResolveTable:
    ...
    def resolve(self, task, value, isPath=False):
        ...
        resolved = dict([(name, resolver.resolve(task))
        ...
        resolved = dict([(name.strip('{}'), resolvedValue)
                        for name, resolvedValue in resolved.items()])
        ...
        for expression in _evalRegEx.findall(value):
            try:
                ...
                value = self.pathSensitiveReplace(
                    value, expression, str(resolvedExpression), isPath)
    ...
    def createSimpleResolveTable(self):
        ...
        for tokenData in testTokens:
            resolver.addResolver(
                '{' + tokenData[0] + '}', 'some description', lambda keyword, task: tokenData[1])
        return resolver
