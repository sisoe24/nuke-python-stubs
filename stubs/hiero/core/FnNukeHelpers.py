...
from hiero.core import (Clip, Keys, Format, Sequence, TrackItem, Annotation,
                        Transition, VideoTrack, AnnotationText,
                        EffectTrackItem, AnnotationStroke)
...
from . import nuke
from .FnCompSourceInfo import CompSourceInfo, isNukeScript
...
def _TrackItem_getTransitions(trackItem):
    ...
    inTransition = trackItem.inTransition() if trackItem.inTransition(
    ) and trackItem.inTransition().isEnabled() else None
    outTransition = trackItem.outTransition() if trackItem.outTransition(
    ) and trackItem.outTransition().isEnabled() else None
...
def _TrackItem_addToNukeScript(self,
    ...
    if firstFrame is not None:
        ...
        hiero.core.log.debug('last_frame(%i) =  firstFrame(%i) + startHandle(%i) + (self.duration() -1)(%i) + endHandle(%i)' % (
        ...
        hiero.core.log.debug('first_frame(%i) =  self.timelineIn(%i) - (startHandle(%i) + inTransitionHandle(%i)' %
                             (first_frame, self.timelineIn(), startHandle, inTransitionHandle))
    ...
    metadataNode.addMetadata(
        [('hiero/shot', self.name()), ('hiero/shot_guid', _guidFromCopyTag(self))])
    ...
    if seq:
        ...
        seqTimecode = hiero.core.Timecode.timeToString(
            seqTimecodeFrame, seq.framerate(), hiero.core.Timecode.kDisplayTimecode)
        ...
        metadataNode.addMetadata([('hiero/project', clip.project().name()),
                                  ...
                                  ('hiero/sequence/timecode', '[make_timecode %s %s %d]' %
                                   (seqTimecode, str(seq.framerate()), first_frame))
                                  ])
    ...
    if includeAnnotations:
        ...
        clipAnnotations = clip.addAnnotationsToNukeScript(
            script, firstFrame=readNodeFirstFrame, trimmed=True, trimStart=readStart, trimEnd=readEnd)
    ...
    timeClipNode = nuke.TimeClipNode(
        first_frame, last_frame, clip.sourceIn(), clip.sourceOut(), first_frame)
    ...
    assert (not includeRetimes) or (
        retimeMethod is not None), 'includeRetimes is true and retimeMethod is None'
    ...
    if includeEffects:
        ...
        linkedEffects = [item for item in self.linkedItems(
    ...
    needAdditionalReformatNodes = (not itemSetToSequenceFormat) and (
        outputToSequenceFormat or linkedEffects)
...
def createAnnotationsGroup(script, annotations, offset, inputs, cliptype=None):
    ...
    # the root node.
    annotationsGroup = nuke.GroupNode(
        'Annotations', disable='{{!annotations_show }}', inputs=inputs)
    ...
    for annotation in annotations:
        annotation.addToNukeScript(annotationsGroup, offset=offset,
                                   inputs=inputs, cliptype=cliptype)
...
def _addEffectsAnnotationsForTrack(track, includeEffects, includeAnnotations, script, offset, inputs=1, cliptype=None):
    ...
    if includeEffects:
        ...
        timecodeStr = hiero.core.Timecode.timeToString(
            timecodeFrame, sequence.framerate(), hiero.core.Timecode.kDisplayTimecode)

        metadataNode = nuke.MetadataNode(inputs=inputs)
        metadataNode.addMetadata([('hiero/sequence/timecode', '[make_timecode %s %s %d]' %
                                 (timecodeStr, str(sequence.framerate()), scriptFrame))])
        script.addNode(metadataNode)
        added_nodes.append(metadataNode)
...
def _VideoTrack_addToNukeScript(self,
    ...
    # Check that we are on the right type of object, just to be safe.
    assert isinstance(
        self, VideoTrack), 'This function can only be punched into a VideoTrack object.'
...
def _Sequence_addToNukeScript(self,
    ...
    connectedTracks, disconnectedTracks = getConnectedDisconnectedTracks(
        self, masterTrackItem.parent(), disconnected, includeEffects, includeAnnotations)
            ...
            if track in tracksWithVideo:
                if track.isBlendEnabled():
                    ...
                    if track.isBlendMaskEnabled():
                        ...
                        # input 'A' is has 0-based index '1' (2nd input) plus the offset for the 'none' entry
                        inputAId = 1 + 1
                        # set by ID instead of string because the enum knob requires fromScript() to parse strings
                        merge.setKnob('maskProviderInput', inputAId)
                        # set the mask to use the upstream alpha channel - use string as the ChannelKnob supports it
                        merge.setKnob('maskChannelInput', 'alpha')
                else:
                    merge = nuke.CopyNode()
                if previousTrack:
    ...
    if outputToFormat:
        ...
        added_nodes.append(outputToFormat.addToNukeScript(
            script, resize=nuke.ReformatNode.kResizeNone, black_outside=False))
        script.popLayoutContext()
...
def _Format_addToNukeScript(self, script=None, resize=nuke.ReformatNode.kResizeWidth, black_outside=True):
    ...
    reformatNode = nuke.ReformatNode(resize=resize, to_type=nuke.ReformatNode.kToFormat,
                                     format=formatstring, pbb=True, black_outside=black_outside)
    if script is not None:
        script.addNode(reformatNode)
...
_hJustifyTable = {AnnotationText.eHLeft: 'left', AnnotationText.eHCenter: 'center',
                  AnnotationText.eHRight: 'right', AnnotationText.eHJustify: 'justify'}
_vJustifyTable = {AnnotationText.eVBaseline: 'baseline', AnnotationText.eVTop: 'top',
                  AnnotationText.eVCenter: 'center', AnnotationText.eVBottom: 'bottom'}
...
def _AnnotationStroke_writeCurveStr(stroke, curve_id):
    ...
    curveStr = (
        ...
        '\n{ a bs %s h 0.9 r %s g %s b %s a 1 opc %s }}' % (
            stroke.lineWidth(), color[0], color[1], color[2], color[3])
    )
    return curveStr
...
def _AnnotationStrokes_addToNukeScript(strokes, script, **knobs):
    ...
    curvesKnob = curvesKnob + '\n'.join([_AnnotationStroke_writeCurveStr(stroke, index + 1)
