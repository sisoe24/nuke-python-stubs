                ...
                if seq:
                    ...
                    for track in list(seq.items()):
                        if isinstance(track, hiero.core.VideoTrack):
                            ...
                            effectItems = [i for i in itertools.chain(
                                *track.subTrackItems()) if isinstance(i, hiero.core.EffectTrackItem)]
...
def findItemsInBin(rootBin, filter=None, partialName=None, verbose=0):
    ...
    if rootBin == None or type(rootBin) != hiero.core.Bin:
        raise Exception(
            'Please provide a valid hiero.core.Bin object as the first argument to hiero.core.findItemsInBin()')
