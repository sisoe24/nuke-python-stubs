...
from hiero.core import (Clip, Keys, ITask, Sequence, TrackItem, AudioTrack,
                        Transition, VideoTrack, ITaskPreset, ItemWrapper,
                        MediaSource, ResolveTable, ApplicationSettings, log,
                        util, remapPath, defaultFrameRates,
...
class TaskBase(ITask):
    ...
    def validate(self):
        ...
        if multiViewProject and not self.views():
            raise RuntimeError('{} does not support multiple views'.format(
                self.ident().split('.')[-1]))
            ...
            if self._cutHandles is None:
                ...
                if inTransition is not None and not inTransition.isNull():
                    if inTransition.alignment() == Transition.kDissolve:
                        ...
                        inTransitionHandle = (ti.timelineIn() - inTransition.timelineIn())
                        inHandle += inTransitionHandle
                        log.debug('  inTransitionHandle = ' + str(inTransitionHandle))
...
class RenderTaskPreset(TaskPresetBase):
    ...
    def _setCodecSettings(self, codecType, extension, fullname, isVideo, properties):
        ...
        self._codecSettings[codecType] = {
            'extension': extension, 'fullname': fullname, 'properties': properties, 'isVideo': isVideo}
    ...
    def addCustomResolveEntries(self, resolver):
        ...
        resolver.addResolver('{ext}', 'Extension of the file to be output',
                             lambda keyword, task: self.extension())
