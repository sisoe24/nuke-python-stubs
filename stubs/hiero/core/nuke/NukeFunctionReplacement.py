...
if nuke.env['studio']:
    ...
    def scriptOpenReplacementFunc(filePath):
        ...
        processWillBeFork = root is not None and (
            root.modified() or isSavedScript) and len(root.nodes()) > 0
...
def _getWorkspaceActions(returnSaveActions):
    ...
    workspaceActions = [
        a for a in workspaceActions if a.objectName().startswith('foundry.workspace.')]
    if returnSaveActions:
        workspaceActions = [a for a in workspaceActions if a.objectName(
        ).startswith('foundry.workspace.save.')]
    else:
        workspaceActions = [a for a in workspaceActions if not a.objectName(
        ).startswith('foundry.workspace.save.')]
...
nuke.restoreWindowLayout = deprecated(restoreWorkspaceFunc, funcName='nuke.restoreWindowLayout',
                                      msg="""This method is deprecated. The Restore action in the Workspace Menu corresponding to the input argument will be triggered.\nhiero.ui.setWorkspace(name) should be called with the desired workspace name.""")


saveWorkspaceFunc.__doc__ = nuke.saveWindowLayout.__doc__
saveWorkspaceFunc.__name__ = nuke.saveWindowLayout.__name__
nuke.saveWindowLayout = deprecated(saveWorkspaceFunc, funcName='nuke.saveWindowLayout',
                                   msg="""This method is deprecared. The Save action in the Workspace Menu corresponding to the input argument will be triggered.\nhiero.ui.saveWorkspace(name) should be called with the new workspace name.""")
