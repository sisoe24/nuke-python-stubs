...
class ScriptWriter:
    ...
    def _postProcessNodes(self):
        ...
        for node in self._nodes:
            ...
            isDisconnectedUserNode = isinstance(
                node, hiero.core.nuke.UserDefinedNode) and nonNodeTreeNodes
...
def getInteractiveNukeExecutablePath():
    ...
    if not util.filesystem.exists(nukePath):
        raise RuntimeError(
            "The specified Nuke application path does not exist (\"%s\")." % (nukePath, ))
    return nukePath
