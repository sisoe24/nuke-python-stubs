    ...
    def serialize(self, serializer, nodeType, knobValues, userKnobs, rawKnobs):
        ...
        for knobKey in list(knobValues.keys()):
            if knobKey not in priorityKnobs:
                serializer.serializeKnob(mappedKnobs[knobKey] if knobKey in list(
                    mappedKnobs.keys()) else knobKey, knobValues[knobKey])
...
class AddTimeCodeNode(Node):
    def __init__(self, timecodeStart, fps, dropFrames, frame=None, metafps=False, inputs=None, inputNode=None):
        ...
        if dropFrames and not fps.supportsDropFrames():
            raise Exception(
                "Adding a time code with drop frames enabled, but the fps (%f) does not support drop frames (meaning it's not 29.97 or 59.94)" % fps.toFloat())
        ...
        if dropFrames and fps.supportsDropFrames():
            startcode = hiero.core.Timecode.timeToString(
                timecodeStart, fps, hiero.core.Timecode.kDisplayDropFrameTimecode)
            fpsKnobValue = fps.toFloat()
        else:
            startcode = hiero.core.Timecode.timeToString(
                timecodeStart, fps, hiero.core.Timecode.kDisplayTimecode)
        ...
        Node.__init__(self, 'AddTimeCode', startcode=startcode, frame=frame,
                      metafps=metafps, useFrame=useFrame, fps=fpsKnobValue, inputNode0=inputNode)


class AppendClipNode(Node):
    def __init__(self, inputs, inputNode=None, **kwargs):
        Node.__init__(self, 'AppendClip', inputNode0=inputNode,
                      inputs=inputs, meta_from_first='false', **kwargs)
...
class MergeNode(Node):
    def __init__(self, inputs, inputNodes=None):
        ...
        Node.__init__(self, 'Merge2', inputNodes=inputNodes, inputs=inputs,
                      metainput='All', operation='over', also_merge='all')
    ...
    def __init__(self, resize=kResizeWidth, center=True, flip=False, flop=False, turn=False, to_type=kToFormat, format=None, scale=None, filter=None, box=None, pbb=False, black_outside=True):
        ...
        if to_type == self.kToFormat:
            ...
            if scale is None or not float(scale) > 0:
                raise ValueError(
                    'to_type is set to kToScale but scale is not a positive floating point number.')
            self.setKnob('scale', scale)
        elif to_type == self.kToBox:
            if box is None or len(box) != 3:
                raise ValueError(
                    'to_type is set to kToBox but box is not a tuple of (width, height, pixel aspect).')
...
class RootNode(Node):
    ...
    def __init__(self, first_frame, last_frame, fps=None, showAnnotations=False):
        Node.__init__(self, 'Root', first_frame=first_frame,
                      last_frame=last_frame, lock_range='true')
