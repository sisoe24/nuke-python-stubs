...
from hiero.core import Transition, FnNukeHelpers
...
class TrackItemScriptWriter(object):
    ...
    def __init__(self,
        ...
        if outTransition is not None:
            if outTransition.alignment() == Transition.kDissolve:
                ...
                self._outTransitionHandle = (
                    outTransition.timelineOut() - self._trackItem.timelineOut())
    ...
    def writeToScript(self,
        ...
        metadataNode.addMetadata([('hiero/shot', self._trackItem.name()),
                                 ('hiero/shot_guid', FnNukeHelpers._guidFromCopyTag(self._trackItem))])
        ...
        if self._params.includeAnnotations():
            ...
            clipAnnotations = clip.addAnnotationsToNukeScript(
                script, firstFrame=self._readNodeFirstFrame, trimmed=True, trimStart=self._readStart, trimEnd=self._readEnd)
    ...
    def findLinkedEffects(self):
        ...
        linkedEffects = [item for item in self._trackItem.linkedItems(
                ...
                if trackItem.isEnabled():
                    ...
                    for read in readNodeInfo:
                        if read in self._readNodes:
                            ...
                            # Create a new object
                            self._readNodes[read] = ReadNodeInstanceInfo(
                                readNodeInfo[read], 1, 0, 'Read' + str(self._readNodeIDCounter))
...
class SequenceScriptWriter(object):
    ...
    def writeTracks(self,
        ...
        for track in reversed(tracks):
            ...
            if len(trackItems) > 0:
                ...
                addChannelNode = not (
                    track == tracks[0]) and not trackDisconnected and not track.isBlendEnabled()
    ...
    def writeTracksWithJoinViews(self, script, offset, skipOffline, mediaToSkip, disconnected, masterTracks):
        ...
        for view in reversed(self._params.views()):
            ...
            self._readNodeUsageCollator.collateReadNodes(
                connectedTracks + disconnectedTracks, self._params, offset)

        for view in reversed(self._params.views()):
            connectedTracks, disconnectedTracks = tracks[view]
            if connectedTracks or disconnectedTracks:
                script.pushLayoutContext('view', view)
                outputJumpName = self.writeTracks(
                    connectedTracks, disconnectedTracks, script, offset, skipOffline, mediaToSkip)
                if connectedTracks:
    ...
    def writeToScript(self,
        ...
        if self.needJoinViews():
            ...
            self._readNodeUsageCollator.collateReadNodes(
                self._sequence.videoTracks(), self._params, offset)
            ...
            self.writeTracks(connectedTracks, disconnectedTracks, script,
                             offset, skipOffline, mediaToSkip, projectSettings)
            script.popLayoutContext()
