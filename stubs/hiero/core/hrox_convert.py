...
from PySide2.QtXml import QDomElement, QDomDocument
...
_kLookParamToKnobDefinitionsR3D = _kLookParamToKnobDefinitionsShared + [
    ...
    # Note these are separate params in the hrox
    (('red', 'green', 'blue'), 'r3dRGBGain', _convertR3DGain),
...
def _cleanupLookElem(look):
    ...
    if effectValues is None:
        return
    paramsToRemove = [param for param in _findAllInElem(
        effectValues, 'EffectParameter') if param.firstChildElement().attribute('name') != 'colourspacename']
...
def _findClipFirstTrackItem(clip):
    ...
    try:
        trackItemGuid = _getElemGuid(_findFirstInElem(
            clip, 'videotracks/VideoTrack/trackItems/TrackItem'))
        return trackItemGuid, True
    except:
        trackItemGuid = _getElemGuid(_findFirstInElem(
            clip, 'audiotracks/AudioTrack/trackItems/TrackItem'))
...
def _convertClipXml(clip, root):
    ...
    mediaSrcGuid = _getElemGuid(_findFirstInElem(
        trackItem, 'MediaGroup/groupdata/MediaInstance_Vector/Source'))
